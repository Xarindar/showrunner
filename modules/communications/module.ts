import type { ShellModule } from "@/shell/module-types";

export const manifest = {
  id: "communications",
  label: "Communications",
  href: "/admin/modules/communications",
  icon: "Mail",
  order: 100,
  navigation: { category: "marketing" },
  description: "Message templates, visual email builder, delivery logs, and suppression controls.",
  layout: "standard",
  status: "active",
  enabledByDefault: true,
  readiness: {
    level: "partial",
    mode: "mixed",
    summary: "Transactional outbox, event-triggered email templates, booking template settings, and structured visual email editing are live; manual notes remain admin-managed.",
    primaryGap: "Campaign scheduling, SMS, and delivery-health dashboards are pending."
  },
  capabilities: [
    { label: "Transactional outbox", status: "live" },
    { label: "Event-triggered templates", status: "live" },
    { label: "Booking template settings", status: "live" },
    { label: "Visual template builder", status: "live" },
    { label: "Manual template catalog", status: "foundation" },
    { label: "Campaign send UI", status: "planned" }
  ],
  adminRoutes: ["/admin/modules/communications"],
  publicRoutes: ["/unsubscribe/[token]", "/api/newsletter/subscribe"],
  dependencies: ["settings", "clients"],
  dataModels: ["MessageTemplate", "MessageLog", "EmailOutbox", "EmailSenderIdentity", "SuppressionListEntry"],
  permissions: ["communications:manage"],
  settingsSections: ["Email", "Integrations"],
  healthChecks: ["smtp-env", "email-worker", "failed-outbox", "sender-verification"]
} satisfies ShellModule;
