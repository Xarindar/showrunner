...
| 10/12 | Visual email template builder + automation template picker | 🛠 RESOLVED | Core was ✅ CONFIRMED 06-09-26 (MJML renderer ratified+wired, output sanitized; clone + MessageTemplateVersion/restore + sample preview). Fresh LINTER pass [06-14-26] raised 8 findings; PATCHER resolved all (see §10/12 body block) — awaiting VALIDATOR. @user decision: system templates stay editable, every email editable in ONE place → consolidated onto the visual builder (booking-settings panel + `updateBookingTemplateSettingsAction` + `booking-templates.ts` deleted; version history moved into builder; `htmlBody` is now only the builder-rendered cache, killing the silently-dead second editor). Deduped `hasBuilderJson`, restore now re-validates tokens, clone dead-spread + `optionalColorSchema` `.catch()` fixed. 🟢 fwd: `mjml2html` cast + snapshot last-write-wins left as-is | 06-14-26 |
