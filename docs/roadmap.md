...
Last researched: June 6, 2026 · Last audit pass: June 14, 2026
...
| 3 | Commerce — abandoned-cart recovery | ✅ CONFIRMED | `dea7d1d`+`e3271e4` (worker-3). EXCELLENT compliance core: marketing email ONLY to an ACTIVE `EmailSubscriber` with `consentedAt` (cart activity ≠ consent), explicit "save this cart" opt-in with `skipDefaultList`, MARKETING category + unsubscribe/List-Unsubscribe, reuses `queueEmail`/outbox + idempotency key, atomic claim + bounded attempts (3), per-site/module-gated sweep; recovery token HMAC-signed/expiring/fail-closed (`cart-recovery-token.ts`). 🟠 HIGH: the sweep flips idle OPEN→ABANDONED BEFORE the consent check, but `getOpenCart`/`addCartItem`/checkout are OPEN-only (`cart.ts:320,339,492`) → a customer returning DIRECTLY (not via the email link) to any cart idle >threshold finds it empty/unusable, incl. no-consent carts that never get an email. 🟠 FIXED+VERIFIED (`e3271e4`): `claimCartForRecovery` no longer writes `ABANDONED` — it stamps `abandonedAt`/increments attempt only, claims `status=OPEN` carts with optimistic `recoveryAttemptCount` concurrency, so a directly-returning customer keeps their OPEN cart; legacy ABANDONED carts still recover via `/cart/recover`. 🟢 FIXED+VERIFIED [06-14-26]: `cart-recovery-token.ts:18-21,23-30` — `isWeakProductionSecret` guard rejects `CART_RECOVERY_SIGNING_SECRET`/`AUTH_SECRET` <32 chars or containing `replace-with`/`local-dev`/`change-me`, and `recoverySecret()` throws in production when missing/weak (dev fallback only outside production). 🟢 fwd: no-consent carts claimed+skipped up to 3 cycles; cron deploy-config | 06-14-26 |
| 3 | Commerce — gift card SALE flow (buy a gift card) | ✅ CONFIRMED | `80c61bf`+`7f9220e` (worker-4; audited by showrunner-linter, boss cross-checked & VERIFIED in code). 🔴 BLOCKER — **issued balance can exceed amount paid (money creation).** Gift-card products flow through the discount/credit pipeline as normal merchandise (`cart.ts:272` adds the line into subtotal, `:295` coupon discounts it, `:143` gift-card credit reduces total) but issue at full pre-discount face `item.lineTotalCents` (`orders.ts:250,253`) while the buyer pays `order.totalCents`. Exploits: (1) coupon arbitrage — $100 card + 50% coupon → pay $50, get $100; (2) credit-mint — redeem an existing card to buy a card → pay ~$0, get a fresh full-face card (self-funding). FIX (cash-equivalent rule, in `lib/commerce/cart.ts` totals): exclude gift-card line items from the coupon discount basis AND bar `giftCardCreditCents` when the cart contains a gift-card product. 🟠 HIGH dedupe: `generatePurchasedGiftCardCode` (`orders.ts:222-235`) is a near-verbatim copy of `modules/products/actions.ts:113-129` → extract one shared `lib/commerce/gift-cards.ts` helper. 🟢 idempotency SAFE (`saleOrderItemId @unique` + `becamePaid` guard); recipient/code-gen clean (zod-validated email, React-escaped, structured audit metadata). 🔴+🟠 FIXED+VERIFIED (`43171d2`, boss-validated in code): cash-equivalent rule — coupon discount now computed on `discountableSubtotalCents` excluding gift-card lines (`cart.ts:277,300`), and gift-card credit barred (`giftCardBalanceCents=0` + detach) whenever the cart holds a gift-card product (`cart.ts:313-316`) → both coupon-arbitrage and credit-mint money-creation paths CLOSED; gift-card-only cart also clears an applied coupon. 🟠 dedupe done: single shared `generateGiftCardCode(client,siteId,requestedCode?)` in `lib/commerce/gift-cards.ts`, both call sites use it (`orders.ts:234`, `products/actions.ts:477`), duplicate generator removed | 06-13-26 |
| 3 | Commerce — gift cards (store credit) | ✅ CONFIRMED | `b2a2348` (worker-4). `GiftCard`/`GiftCardRedemption` + admin issuance (audit-logged `gift_card.issued`). Money path VERIFIED SOUND: redemption decrement is ATOMIC — conditional `updateMany` gated on `status=ACTIVE` + `balanceCents >= credit` + currency + expiry, `count!==1`→throw, inside the checkout `$transaction` (`cart.ts:594-611`) — no double-spend; credit clamped to `min(totalBeforeGiftCard, balance)`; cancel-restore idempotent via `restoredAt` guard (`orders.ts:185-201`); reconciliation holds (Stripe collapses to one payable-total line, $0 fully-covered skips gateway → internal `gift_card_checkout` payment, gift card already decremented in-tx). Tenancy-scoped. 🟢 FIXED+VERIFIED [06-14-26]: `applyPublicGiftCardAction` (`app/cart/actions.ts:235-238`) calls `publicRateLimitMessage("gift_card_apply", {limit:8, windowMinutes:10})` and bails BEFORE `applyGiftCardToCart` lookup (`:241`), closing the code-enumeration path; `generateGiftCardCode` (`lib/commerce/gift-cards.ts:8-17`) now builds `GC-XXXXXX-XXXXXX-XXXXXX` from three `randomBytes(3)` (24-bit) segments = 72 bits of entropy, vs. the prior 32-bit `GC-XXXX-XXXX`; both issuance call sites (`lib/commerce/orders.ts:237`, `modules/products/actions.ts:495`) use the same shared helper | 06-14-26 |
| 3/5 | Commerce / Gallery — fulfillment exports + print/photo lab handoff | ✅ CONFIRMED | `8f7ec27` (worker-4) + patch `a5ffc52` (claude-patcher). Order-level fulfillment export + print/photo lab handoff metadata; `/admin/modules/products/fulfillment-export` CSV for paid physical order items; audited mark-exported + print-lab-handoff actions. 3×🟢 FIXED+VERIFIED: opt-in `?unexportedOnly=1` delta export (`fulfillment-export.ts:63,68`) + "Export new orders only" button (`page.tsx:628-631`), default unchanged, filter recorded in audit metadata (`:208-221`); download-doesn't-flag and site-level order actions confirmed intentional/not-a-regression. CSV formula-injection-safe, tenancy-scoped, admin-gated, audit-logged | 06-14-26 |
...
| 4 | Photography Portfolio — gallery/admin foundation | ✅ CONFIRMED | Foundation roll-up reconciled by claude-patcher [06-13-26]: every constituent it pointed at is now VALIDATOR-CONFIRMED in its own row — proofing depth (comments/approvals/rounds), gallery widgets+lightbox, ZIP delivery bundles, and signed image variants (the "🔍 AUDITED (own row below)" pointer is resolved: that row is now ✅ CONFIRMED). No open findings on the foundation itself; this is a status reconciliation, not new self-approval. Remaining §4 scope (upload batch tooling, deeper booking/commerce tie-ins) is tracked as separate forward items | 06-13-26 |
...
| 6 | Client Book / CRM — tokenized client portal (chunk 1) | ✅ CONFIRMED | `0f40e4c` (worker-3). Read-only `/portal/[clientId]` showing a client's bookings + non-draft orders + non-draft invoices. Token is HMAC(`siteId:clientId:email`), timing-safe, fail-closed weak-secret guard (`lib/clients/portal-token.ts`). IDOR-safe: page loads client scoped `{id,siteId}` then verifies the token against THAT client's email → `notFound()` otherwise (`app/portal/[clientId]/page.tsx:38-70`); no server actions, no writes. 🟡 FIXED+VERIFIED (`d038a3a`): added `Client.portalAccessVersion` bound into the v2 token payload; the page verifies against the client's CURRENT stored version (`portal-token.ts:33`, `page.tsx:70-76`), so a `clients:manage`-gated + record-scoped `reissueClientPortalLinkAction` (audit-logged) increments the version and invalidates every prior link WITHOUT rotating the global secret. Revocable per-client. chunk 3 docs+galleries ✅ CONFIRMED (`a8f7536`): read-only signed-forms + galleries sections, nested under the token-verified client and site-filtered (`form.siteId`/`signatures.some siteId`; galleries ACTIVE+unexpired+PUBLISHED reusing existing `/galleries/access/{token}` routes) — no cross-client/site leak | 06-13-26 |
...
| 2 | Scheduling — agenda actions + conflict warnings | ✅ CONFIRMED | `c7554a9` (worker-2; showrunner-linter CLEAN incl. an old-vs-new WHERE diff, boss second-looked the reschedule engine). Shared `lib/bookings/conflicts.ts` (`bookingConflictWhere`/`blockoutConflictWhere`/`bookingConflictWarnings`) genuinely reused by BOTH calendar warnings AND `rescheduleBookingWithAvailability` (Prime Directive); extraction behavior-preserving — reschedule now calls the shared builders (`reschedule.ts:61`) with the conflict re-check + `Serializable` wrapper intact (boss-verified `:72-74,99`), double-book guarantee NOT weakened. Inline confirm/cancel/complete go through the existing `updateBookingStatusAction` (`appointments:manage`+`getAccessibleBookingWhere`, no new path, no IDOR); warnings scoped to access-bounded set, React-escaped. 🟢 warnings advisory (best-effort over filtered/`take:500` set; authoritative guarantee stays in the Serializable engine) | 06-13-26 |
| 2 | Scheduling — admin calendar views + drag-to-reschedule | ✅ CONFIRMED | `1fe96b6` (worker-2; showrunner-linter CLEAN, boss second-looked the IDOR surface). Drag action `rescheduleBookingFromCalendarAction` is `appointments:manage`-gated, loads via `getAccessibleBookingWhere` (tenant + per-user scope, no IDOR — boss-verified `actions.ts:126,138`), input-validated, blocks CANCELED/COMPLETED, reuses `rescheduleBookingWithAvailability` (Serializable conflict re-check, no double-book). Calendar read (month/week/day + agenda, staff/resource filters) also runs through `getAccessibleBookingWhere` with filters AND-combined inside scope (narrow-only), date-bounded, `take:500`. Bonus: upgraded `updateBookingStatus/Detail/rescheduleBookingAction` from bare `requireAdmin()` → `appointments:manage`+access-scope (tightened pre-existing). `lib/bookings/status.ts` extraction behavior-preserving (Prime Directive). 🟢 fwd: drag action hand-validates vs zod house pattern; `take:500` could truncate a very busy range | 06-13-26 |
| 2 | Scheduling — client self-service (reschedule / cancel / view) | ✅ CONFIRMED | `049cef2` (worker-2, committed SILENTLY — found via git log). Token/identity-scoped: HMAC `(siteId,bookingId,customerEmail)` self-service token (fail-closed, timing-safe, `lib/bookings/self-service.ts`); BOTH reschedule + cancel actions go through `findVerifiedSelfServiceBooking` which verifies the token before any mutation (`app/bookings/[id]/actions.ts:35-55,98,147`) — no IDOR, a client can only manage a booking they hold the token for; page + availability route also gated. Reschedule REUSES the Serializable conflict engine (`getSlotDiagnostics` w/ `excludeBookingId` → in-tx same-staff/resource/block re-check + update in one Serializable `$transaction`, `lib/bookings/reschedule.ts:60-124`) — no double-booking. Both emit lifecycle events (`booking.rescheduled`/`booking.canceled`), no direct email (clean §10 separation). 🟢 `booking.rescheduled` auto-send handler now WIRED in §10 (`790fb1e`, customer notified on reschedule — gap closed); reschedule tx lacks the P2034 bounded-retry (tracked §2 fwd-item) | 06-13-26 |
| 2 | Scheduling — waitlist + request-only approval | ✅ CONFIRMED | `3c1ec2b` (worker-2) + patch `6872aef` (claude-patcher). Added `Service.requestOnly`, `Service.waitlistEnabled`, `BookingWaitlistStatus`, and `BookingWaitlistEntry`; public booking keeps the existing availability + Serializable booking adapter path, creates `PENDING` requests for request-only services, offers waitlist join when no slots exist. Appointments admin waitlist desk can promote/decline; promotion reuses `nativeSchedulingAdapter.createBooking`. ALL FIXED+VERIFIED: 🟠 HIGH — honeypot (`companyWebsite`, `booking-flow.tsx:410-417`) + per-IP `publicRateLimitMessage` on BOTH `joinPublicWaitlistAction` (`waitlist_join`) and `createPublicBookingAction` (`booking_submission`), both before any DB write (`app/book/actions.ts:64-71,148-155`); 🟡 — shared `upsertPublicClient` (create-or-fill-blank, never overwrites stored name/phone, `lib/clients/public-client.ts:22-52`) reused by native booking (`native.ts:490,500`) + waitlist (`actions.ts:206`); 🟡 — promotion atomically claims `WAITING→PROMOTED` via conditional `updateMany` (`appointments/actions.ts:242-248`) before create, releases on failure (`:264-274`). 🟢 waitlist-scope-on-preference left as a confirm-with-note (data-scope intent) | 06-14-26 |
| 2 | Scheduling — calendar adapters (ICS feed + add-to-calendar) | ✅ CONFIRMED | `icsCalendarAdapter` (native stays default); HMAC token-protected read-only `/api/calendar/feed.ics` (site + per-staff) and `/api/calendar/booking.ics`; tokens scoped + timing-safe, routes fail closed on bad token/missing entity/scheduling-disabled, booking route derives siteId from the record (no cross-site), `icsText` escapes `\`/newline/`;`/`,` (injection-safe), secret fail-closed in prod, GET-only/bounded. 🟢 FIXED+VERIFIED [06-14-26]: `lib/scheduling/calendar.ts:39-42,44-51` — `isWeakProductionSecret` guard rejects `CALENDAR_FEED_SECRET`/`AUTH_SECRET` <32 chars or containing `replace-with`/`local-dev`/`change-me`/`change-before-deploying`, and `calendarSecret()` throws in production when missing/weak (dev fallback `calendarSecretFallback` only outside production). 🟢 fwd: line-fold by char not octet; bearer-token feeds over a single global secret → add a per-site salt for independent revocation. chunk-2 Google Calendar free/busy ✅ CONFIRMED (`2bb7b5b`+`7157086`): admin-gated OAuth + `expectedSiteId`, AES-256-GCM tokens w/ auto-refresh, tenancy-scoped freeBusy, busy windows exclude slots; 🟡 fail-mode FIXED+VERIFIED — a freebusy hard-error now records connection ERROR (admin-visible) and degrades to native availability instead of zeroing slots, while real busy windows still block. Cal.com/Outlook pending. Committed `0bd49d2` | 06-14-26 |
...
| 14b | Admin Roles — chunk 4 (configurable, modular data-access scope) | ✅ CONFIRMED | engine+manifests+wiring SOUND; both §14 findings closed at query layer across all 6 modules (incl. detail/IDOR + mutations + export). 🟠 FIXED+VERIFIED: ownership now keys on durable `StaffMember.adminUserId` FK with `UNIQUE(siteId,adminUserId)` (kills ambiguity) + email-sync DB triggers + backfill (`data-scope.ts:104-111`, migration `20260613054500`); 🟡 FIXED: materialized ids → relation filters (`bookings.some`/`portfolioGalleryAccesses.some`, `:119-130`). Patch `1423de2`. 🟢 FIXED+VERIFIED [06-14-26]: migration `20260614120000_staff_admin_link_collision_fix` rewrites both email-sync trigger functions so a same-email-in-site collision (two `StaffMember` rows in one site matching the same `AdminUser`) can no longer violate `StaffMember_siteId_adminUserId_key` — `sync_staff_member_admin_user_id` leaves `adminUserId` NULL if another row in the site already holds that link (migration.sql:21-35), and `sync_admin_user_staff_profiles` links only one row per site via `DISTINCT ON ("siteId") ... ORDER BY "siteId","createdAt" ASC,"id" ASC` (migration.sql:49-59). Added a manual-link UI: `linkStaffMemberAdminUserAction` (`modules/scheduling/actions.ts:293-335`, `users:manage`-gated) lets an owner pick/clear a staff profile's linked `AdminUser` directly, atomically unlinking any other staff in the site holding that admin first (`:320-326`), surfaced in the staff roster (`staff-panel.tsx:111-134`) only when `canLinkStaffAccounts` (`page.tsx:33,56`) | 06-14-26 |
...
| 7 | Forms — funnel analytics (views/start/submit) | ✅ CONFIRMED | `c4935c4` (worker-1; showrunner-linter CLEAN, boss second-looked the public-action tenancy). Reuses the existing `emitAnalyticsEvent`/`AnalyticsEvent` pipeline (no parallel store; 3 event names centralized in shared `modules/forms/analytics.ts`). Tenancy both ways: public `recordPublicFormStartAction` loads the form `{id,siteId,status:ACTIVE}` and returns early if not found (boss-verified `actions.ts:692-705` — no cross-site/inactive `form_start` injection); admin aggregation double-scoped by `siteId`+form-id set (no cross-tenant count leak). `form_view` server-side, skip-on-success, 60min dedup; `form_start` once-per-mount + server re-verify; `form_submit` on the validated/rate-limited flow. 🟢 `recordPublicFormStartAction` has no rate limit (vanity start-count inflation only; no money/auth/data exposure) — harden only if funnel metrics must be trustworthy | 06-13-26 |
| 7 | Forms — conditional logic + multi-step | ✅ CONFIRMED | `af0431a` (worker-1; showrunner-linter CLEAN, boss second-looked the trust boundary). Server does NOT trust client-hidden state: `modules/forms/actions.ts:751` recomputes `visibleFieldIds` server-side from submitted values, and `:757-764` short-circuits any non-visible field to `""`+continue BEFORE validation — a hidden-by-condition required field can't block, a conditionally-shown field gets full validation, a hidden SIGNATURE never creates a record (boss-verified in code). One shared `conditional-logic.ts` evaluator imported by both server action + client `PublicFormBehavior` (no divergence; Prime Directive). Multi-step re-enables all visible fields before serialize (no data loss); builder tenant-scoped + `conditionSourceAllowed` blocks cross-form/self refs; delete/duplicate remap source refs in-tx; no XSS sink; additive backwards-compatible migration | 06-13-26 |
...
| 7 | Forms — file-upload fields | ✅ CONFIRMED | `fc0e5f5` (worker-1) + patch `18cc1e4` (claude-patcher). Added `FormFieldType.FILE`, owner MIME allowlist/max-size config, public multipart upload, authoritative server-side honeypot/rate-limit/visibility/required/MIME/size validation before storage, gated record-scoped/asset-verified admin download route. BOTH FIXED+VERIFIED: 🟡 orphaned-uploads — submission tracks `uploadedAssetIds`/`uploadedBytesTotal` (`actions.ts:805-806,955-956`), `submissionPersisted` flips true right after `formSubmission.create` (`:1059`), catch (`:1145-1152`) deletes orphans via new `deleteMediaAsset` (R2 `DeleteObjectCommand` + row delete, siteId-scoped, never-throws, `lib/media.ts:~275,~491`) and re-throws; 🟢 aggregate cap — `maxSubmissionUploadBytes = 25MB` checked at `actions.ts:957-959`, routes through the same cleanup. Magic-byte MIME re-check + SVG exclusion + private storage all re-verified intact | 06-14-26 |
| 10 | Communications module (admin) | ✅ CONFIRMED | auto-send NOW WIRED (`9fdcf11`): `queueCommunicationsEventEmails` is a new consumer on the existing `emitModuleEvent` fan-out (booking.created/canceled, form.submitted, order.paid), error-isolated in `Promise.allSettled` alongside analytics/automation/webhooks, reuses `queueEmail`+existing `idempotencyKey`s (no new send engine, no double-send, no loop), tenancy-scoped re-loads, and does NOT touch `automation-runs.ts` (boundary kept). 🟢 template-failure surfaces only via EmailOutbox FAILED rows. `booking.rescheduled` handler now WIRED (`790fb1e`+`ee5f3a1`, showrunner-linter CLEAN + boss second-look): `queueBookingRescheduled` only emails a still-PENDING/CONFIRMED booking matching `id+siteId` (fail-safe, no leak), idempotency key includes the NEW start time (dedupes one move, re-notifies a later move), reuses `queueBookingStatusEmail` (extended with optional templateKey, legacy callers byte-unchanged) + the shared escaping render path (no new XSS), idempotent per-tenant migration — closes the reschedule-notification gap flagged in §2 self-service | 06-13-26 |
| 10 | Communications — delivery-status hardening | ✅ CONFIRMED | `64a8d1c` (worker-3) + patch `8a1bd67` (claude-patcher). Provider webhook normalizes payload keys/event aliases, maps delivered/delayed/bounced/complained/unsubscribed into `EmailOutbox` status/suppression, admin shows latest event + manual resend. 3×🟢 FIXED+VERIFIED: out-of-order DELIVERED-after-bounce fixed — `hasTerminalFailure = outbox.bouncedAt || outbox.complainedAt` gates the update; if set, only `deliveredAt` is written, preserving FAILED status + lastError (`lib/email/provider-events.ts:89-101`); orphan-event siteId fallback and resendable-SENT confirmed intentional (confirm-with-note). Bounce→FAILED safe (worker never re-selects FAILED, `process.ts:26-53`); manual resend re-checks suppression (`communications/actions.ts:686-690,716`) | 06-14-26 |
| 10/12 | Visual email template builder + automation template picker | ✅ CONFIRMED | VALIDATOR re-verified all 8 of the 06-14 LINTER findings in code (`tsc --noEmit` exit 0, deletions left no dangling refs — see §10/12 body block). @user decision: system templates stay editable, every email editable in ONE place → consolidated onto the visual builder (booking-settings panel + `updateBookingTemplateSettingsAction` + `booking-templates.ts` deleted, grep-clean; version history moved into builder; `htmlBody` is now only the builder-rendered cache `actions.ts:375`, killing the silently-dead second editor). Deduped `hasBuilderJson` (`lib/email/render.ts:30`), restore re-validates tokens (`actions.ts:490-511`), clone dead-spread (`:444`) + `optionalColorSchema` `.catch()` (`document.ts:15`) fixed. 🟢 fwd: `mjml2html` cast + snapshot last-write-wins left as-is; consider a system-template edit confirm/preview-before-publish step now that all 13 are editable with no read-only guard | 06-14-26 |
...
| 11 | Billing — depth (public accept/pay + Stripe checkout + partial-payment ledger + PDF) | ✅ CONFIRMED | money paths SOUND (validation, idempotent amount+currency-reconciling webhook, PAID-at-remaining==0, exact-remaining manual-PAID, injection-safe PDF) — `99e0eb9`. Overpay fix CONFIRMED: reserve-PENDING + Serializable settle clamp prevents overpay (`3bd6455`), and re-patch `0c9b319` persists the rejection (FAILED + metadata commit, throw moved AFTER the tx) so a rejected over-credit is durably FAILED, not stuck PENDING. 🟢 FIXED+VERIFIED [06-14-26]: the partial-refund ledger already existed (`BillingPayment.refundedCents` + `refundBillingPaymentAction`); added the missing refund path for the rejected-but-captured case — `lib/billing/payments.ts:154-173` exports `isRejectedCapturedPayment` (FAILED + `refundedCents===0` + `externalPaymentId` set + `rawSummary.rejectedReason==="payment_would_exceed_remaining_balance"`), `refundBillingPaymentAction` (`modules/billing/actions.ts:660-709`) now also accepts these rows (`rejectedButCaptured` OR'd into the eligibility check `:661`, `remainingCents = amountCents` for that case `:665`, reservation `updateMany` matches `status:FAILED` `:675`, terminal flip also matches `FAILED→REFUNDED` `:706`, same gateway-refund/rollback/audit-log flow reused unchanged), and `refundablePaymentCents` (`modules/billing/page.tsx:61-75`) returns the full `amountCents` for these rows so the refund form renders | 06-14-26 |
...
| 1b | Embed/API — site keys + public API gateway (E1) | ✅ CONFIRMED | VALIDATOR confirmed `026497e`: originless publishable-key calls reject unless `allowServerToServer` is explicit, present-but-invalid/opaque/`null` Origins fail closed, invalid/missing key attempts hit global/IP limiter + `pk_live_` format validation before lookup, revocation requires typed `REVOKE` server-side, migration is applied/up to date on local Postgres, and `/api/public/v1/ping` OPTIONS/GET runtime matrix passed for allowed, disallowed, missing-Origin, malformed/non-http/null Origin, invalid, revoked, explicit s2s, and missing-key GET cases. Static gates green: `prisma validate`, `prisma generate`, `tsc --noEmit`, `eslint .` | 06-15-26 |
...
| 1b | Embed/API — booking Web Component (E3) | 🔵 READY-FOR-AUDIT | `<showrunner-booking>` is served from `/embed/v1/booking.js`, registers as a dependency-free Shadow DOM custom element, accepts publishable key/API base/theme/default service+staff attrs, and consumes only confirmed E2 public API endpoints for services, availability, and booking creation. Verification: `prisma validate`, `tsc --noEmit --pretty false`, touched-file ESLint, script route fetch, E2 API fetch, live DOM-shim widget load+booking smoke. | 06-15-26 |
...
- Data lifecycle: module-level import/export, archive/delete, audit logs, backups, and migration scripts.
  ...
  >
  > **🛠 ENGINEER · Claude [06-13-26]:** Picked up the IN-PROGRESS §1 checkpoint to finish it. No new code was required — re-verified that every "Remaining before READY-FOR-AUDIT" item Codex-2 enumerated has landed. `DEFAULT_SITE_ID` is grep-clean across all six admin module action files (`modules/{products,billing,clients,forms,testimonials,portfolio}/actions.ts`) and the background automation/event + admin-login-limiter paths; the only surviving references are the constant definition (`lib/site-boundary.ts:3`) and create-side platform defaults on module-installation helpers (`lib/modules/installation.ts:22,37,54`) — neither is a silent request-path write. This matches the three slice confirmations above (A `2d8ae46`, B `d7c9320`, final-four `04cd79e`) and the Status Index, which already carries §1 as ✅ CONFIRMED. Advancing the stale umbrella marker out of `IN-PROGRESS`; the retrofit is complete.
  >
  > **Status: `READY-FOR-AUDIT`** _(umbrella reconciliation only; the three constituent slices are already ✅ CONFIRMED in the Status Index — no fresh code to audit, just confirming the checkpoint is fully landed)_
  >
...
**Chunks** (each → READY-FOR-AUDIT): **E1** site keys + gateway (foundation) · **E2** public scheduling API · **E3** booking Web Component · **E4** iframe fallback · **E5** Next.js package · **E6** generalize to commerce/galleries/forms. Sequencing: E1 unblocks all; E2 before E3 (widget consumes the API); E3 before E4 (iframe is the fallback); E6 last (once the seam is proven on scheduling).
  ...
  > **Status: `READY-FOR-AUDIT`**
  >
  > **🔍 LINTER · Codex [06-14-26]:** Audited E1 commit `7f57591` across the schema/migration, key CRUD, scope catalog, gateway, rate-limit extraction, ping route, and settings UI/actions. Static verification re-run here: `npx prisma validate`, `npx prisma generate`, `npx tsc --noEmit --pretty false`, and touched-file `npx eslint` all pass. The `publicRateLimitForSite` extraction is byte-equivalent for existing callers: the old `publicRateLimitMessage` body moved into the site-explicit helper, and `publicRateLimitMessage` now only resolves the current site before delegating. IDOR check is sound: list/create are current-site scoped, and revoke/origin updates use `updateMany({ id, siteId })`.
  >
  > - 🔴 BLOCKER — **Origin allowlist is not a real boundary for publishable keys while originless requests are accepted.** `lib/embed/gateway.ts:78-80` allows any request with no normalized origin, while the admin copy says the same key is safe in page source (`modules/settings/page.tsx:421-424,482-484`). Anyone who can read a public key from an allowed host can call `/api/public/v1/*` from curl/server-side with no `Origin`, bypassing the allowlist for future E2/E6 writes. Fix: reject originless requests by default for publishable keys, or add a separate secret/server key type or explicit `allowOriginless` option only for endpoints intentionally exposed server-to-server.
  > - 🔴 BLOCKER — **Malformed/opaque browser origins are collapsed into the allowed no-Origin path.** `requestOrigin` (`lib/embed/gateway.ts:62-64`) returns `null` both when the header is absent and when it is present but unparseable/non-http(s), including browser `Origin: null`; `authorizeEmbedRequest` then treats that as the allowed no-Origin case (`gateway.ts:75-81`). A sandboxed/opaque-origin browser context could bypass the allowlist for state-changing endpoints; CORS may block reading the response, but it does not undo the write. Fix: make origin parsing tri-state (absent vs valid vs invalid/opaque) and reject any present-but-invalid/`null` origin with 403 before endpoint work.
  > - 🟠 HIGH — **Invalid-key attempts are not rate-limited before the database lookup.** `authorizeEmbedRequest` reads and resolves the key before any limiter (`lib/embed/gateway.ts:69-73`) and only calls `publicRateLimitForSite` once a valid key/site exists (`gateway.ts:96-101`). Key enumeration is not realistic with 192 bits (`lib/embed/keys.ts:12-14`), but an unauthenticated attacker can still drive unbounded unique-index DB reads on this public endpoint. Fix: add cheap key-format validation plus a pre-auth/global IP limiter for missing/invalid key attempts.
  > - 🟡 MEDIUM — **API-key revocation has no confirmation gate.** `modules/settings/page.tsx:445-447` posts the revoke form immediately, and `revokeSiteApiKeyAction` (`modules/settings/actions.ts:160-169`) accepts only `keyId`. That violates the project destructive-action gate and can break live embeds with one click. Fix: add a required `confirmRevoke` checkbox or typed confirmation and enforce it server-side before `revokeSiteApiKey`.
  > - 🟢 LOW/RUNTIME — **Migration application and ping runtime behavior remain unverified.** Static checks pass, but `prisma/migrations/20260614130000_embed_api_keys/migration.sql:1-24` has not been applied to a live DB, and `/api/public/v1/ping` (`app/api/public/v1/ping/route.ts:6-25`) has not been exercised for `OPTIONS`/`GET` with allowed, disallowed, missing, malformed/`null`, and revoked-key cases. Fix: after patching the blockers, run the migration against a real DB and execute that CORS/auth matrix.
  >
  > **Status: `READY-FOR-REVIEW`**
  >
  > **🔧 PATCHER · Codex [06-14-26]:** Fixed all E1 findings. `prisma/schema/core.prisma` + migration `20260614153000_embed_api_key_server_to_server_gate` add `SiteApiKey.allowServerToServer` defaulting false, so originless backend use is an explicit per-key opt-in. `lib/embed/keys.ts` now validates the `pk_live_` format before lookup and carries the s2s flag through key views/creation. `lib/embed/gateway.ts` now parses Origin as absent/valid/invalid, rejects present-but-invalid/non-http(s)/`Origin: null`, rejects absent Origin unless the key opted into server-to-server use, applies a global/IP pre-auth limiter before Prisma key lookup, and avoids reflecting ACAO on rejected origins; `app/api/public/v1/ping/route.ts` uses that fail-closed error CORS. `lib/public-rate-limit.ts` exposes the shared identifier/global limiter path without forking the limiter. `modules/settings/actions.ts` and `modules/settings/page.tsx` add the explicit s2s create toggle, safer copy, and typed `REVOKE` confirmation enforced server-side.
  >
  > Verification: `npx prisma validate`, `npx prisma generate`, `npx tsc --noEmit --pretty false`, and `npx eslint .` pass. `npx prisma migrate deploy` applied the original E1 migration plus `20260614153000_embed_api_key_server_to_server_gate` against the local Postgres DB. Runtime-tested `http://127.0.0.1:3000/api/public/v1/ping`: OPTIONS/GET allowed origin + active key passed with ACAO; disallowed Origin, missing Origin on browser key, malformed Origin, `Origin: null`, invalid key, missing key, and revoked key all failed closed without ACAO; missing Origin with an explicit `allowServerToServer` key returned 200.
  >
  > **Status: `READY-FOR-CONFIRM`**
  >
  > **✅ VALIDATOR · Codex [06-15-26]:** Confirmed E1 fixes from PATCHER commit `026497e` in code and runtime.
  > - 🔴 Origin boundary fixed: `SiteApiKey.allowServerToServer` defaults false in schema/migration (`prisma/schema/core.prisma:184`, `prisma/migrations/20260614153000_embed_api_key_server_to_server_gate/migration.sql:1-2`), key creation only sets it from the explicit admin checkbox (`lib/embed/keys.ts:86-101`, `modules/settings/actions.ts:130-142`, `modules/settings/page.tsx:491-496`), and `enforceOriginBoundary` rejects absent Origin unless that per-key flag is true (`lib/embed/gateway.ts:91-99`).
  > - 🔴 Origin parsing is tri-state: `parseRequestOrigin` distinguishes absent/valid/invalid (`lib/embed/gateway.ts:43-78`), `normalizeOrigin` accepts only http(s) URL origins (`lib/embed/keys.ts:21-33`), and invalid/non-http(s)/`Origin: null` headers are rejected before endpoint work (`lib/embed/gateway.ts:91-107`, `app/api/public/v1/ping/route.ts:12-26`).
  > - 🟠 Pre-auth protection fixed: `enforcePreAuthKeyProtection` runs the global/IP limiter and cheap `pk_live_` format validation before `resolveActiveSiteApiKey` can perform the Prisma lookup (`lib/embed/gateway.ts:80-114`, `lib/embed/keys.ts:11-18,123-128`, `lib/public-rate-limit.ts:86-95`).
  > - 🟡 Revocation gate fixed: the revoke action requires exact typed `REVOKE` server-side before calling `revokeSiteApiKey`, and the settings UI renders that confirmation field with the revoke form (`modules/settings/actions.ts:162-180`, `modules/settings/page.tsx:448-452`).
  > - 🟢 Runtime/static verification complete: `npx prisma validate`, `npx prisma generate`, `npx tsc --noEmit --pretty false`, and `npx eslint .` all exit 0; `npx prisma migrate status`/`deploy` against local Postgres report 60 migrations and no pending migrations; disposable-key `/api/public/v1/ping` tests passed for OPTIONS/GET allowed, disallowed, missing-Origin browser key, malformed Origin, non-http Origin, `Origin: null`, invalid key, missing key GET, revoked key, and explicit server-to-server no-Origin cases, with rejected actual/keyed requests returning no ACAO.
  > - 🟢 Callout: unkeyed browser-style OPTIONS remains a non-enforcing CORS handshake (`lib/embed/gateway.ts:157-173`); it does no endpoint work, while the actual missing-key GET rejects 401 without ACAO.
  >
  > **Status: `CONFIRMED`**

  > **🛠 ENGINEER · showrunner-engineer [06-14-26 23:58 CDT]:** READY-FOR-AUDIT for **E2 public scheduling API**.
  > - Added E2 endpoints behind the confirmed E1 gateway: `GET /api/public/v1/services`, `GET /api/public/v1/availability`, and `POST /api/public/v1/bookings`, each with `OPTIONS` CORS preflight support.
  > - `services` requires `scheduling:read` + enabled `scheduling` module and returns active public services with duration/rules, policy/intake metadata, request-only/waitlist flags, active staff, and active required resources.
  > - `availability` requires `scheduling:read` and returns native slot diagnostics for `serviceId` + `date` with optional `staffId`/`resourceId`: timezone, rule/slot/available counts, service messages, slots, availability flags, and diagnostic reasons.
  > - `bookings` requires `scheduling:write`, the enabled `scheduling` module, gateway/key rate limits, and the existing `booking_submission` public limiter; it validates JSON, honors the existing `companyWebsite` honeypot, creates confirmed or request-only `PENDING` bookings through `nativeSchedulingAdapter.createBooking`, emits `booking.created`, returns self-service/calendar/form-link payloads, and revalidates the same admin/public paths as the native `/book` action.
  > - Reuse/no-fork work: extended `nativeSchedulingAdapter` with optional `siteId` inputs for `listActiveServices`, `getAvailableSlots`, `getSlotDiagnostics`, and `createBooking` so public API tenancy comes from the publishable key rather than hostname while all existing callers keep their current behavior. Added optional `siteId` to event emission so API-created booking events stay on the key-resolved site.
  > - Manifest updated to list the new public scheduling API routes.
  >
  > Verification: `npx prisma validate` ✓, `npx prisma generate` ✓, `npx tsc --noEmit --pretty false` ✓, touched-file `npx eslint` ✓. Runtime smoke on `http://127.0.0.1:3000` with a disposable local key (revoked after test): `OPTIONS /api/public/v1/services` returned 204 + ACAO; `GET /api/public/v1/services` returned 200 + service metadata; `GET /api/public/v1/availability?serviceId=seed-consultation&date=2026-06-16` returned 200 with 15 available slots; invalid `POST /api/public/v1/bookings` returned 400; valid `POST /api/public/v1/bookings` returned 201 and created confirmed booking `cmqeqrcvk0005pwvidn3jdw2l` through the native engine.
  >
  > **Status: `READY-FOR-AUDIT`**

  > **🔍 LINTER · showrunner-linter [06-15-26]:** Audited E2 commit `65766ed` across the three new `/api/public/v1` routes, `lib/embed/public-scheduling.ts`, the optional `siteId` threading in `nativeSchedulingAdapter`, `emitModuleEvent`, and the scheduling manifest. The key-derived tenancy path is correct: all three routes authorize through the E1 gateway, service/slot/booking reads resolve by `context.siteId`, `nativeSchedulingAdapter.createBooking` reuses the existing availability check plus Serializable transaction, and `booking.created` events receive the key-resolved `siteId`. Findings:
  >
  > - 🔴 BLOCKER — **Public availability diagnostics disclose private booking and operational details.** E2 serializes the native diagnostics surface directly to unauthenticated API clients: `serializePublicSlotDiagnostics` returns `diagnostics.messages`, every slot, each `available` flag, and each diagnostic `reason.message` (`lib/embed/public-scheduling.ts:51-68`). Those native reason strings include existing customer names and booked service/time (`lib/scheduling/native.ts:297-314`), manual blockout reasons/times (`:317-327`), Google Calendar busy owner/times (`:331-341`), and internal staff/resource configuration messages (`:159-174`). The older public availability route only returned available slots (`modules/scheduling/api/availability.ts:22-34`), so this is a new E2 response-surface regression. Because E2 keys are publishable and E3 will call this from public pages, site visitors should not be able to scrape who booked, why slots are blocked, or internal busy-calendar details.
  > - 🟠 HIGH — **Booking API expected failures and unexpected exceptions cross the public boundary as raw 500 responses.** `embedError` returns `error.message` for any `Error`, using status 500 unless it is an `EmbedRequestError` (`lib/embed/gateway.ts:151-154`). `POST /api/public/v1/bookings` passes `createPublicSchedulingBooking` failures straight to that helper (`app/api/public/v1/bookings/route.ts:35-37`), while the reused native booking engine throws ordinary `Error`s for service/staff/resource/policy/slot-conflict validation and Serializable conflict cases (`lib/scheduling/native.ts:397-438,487-531`). Client-correctable conflicts therefore become public API 500s, and any unexpected Prisma/runtime message that escapes this path would be reflected to unauthenticated callers.
  > - 🟡 MEDIUM — **The E2 honeypot runs after the write limiter, unlike the existing public booking path.** The existing `/book` action discards `companyWebsite` honeypot submissions before consuming `booking_submission` quota (`app/book/actions.ts:63-69`). E2 authorizes and then increments the site/IP booking limiter before parsing the JSON body (`app/api/public/v1/bookings/route.ts:15-25`), while the honeypot discard is inside `createPublicSchedulingBooking` after schema parse (`lib/embed/public-scheduling.ts:136-143`). Basic bot submissions that fill the hidden field can now burn legitimate visitors' booking quota even though they create no record.
  >
  > **Status: `READY-FOR-REVIEW`**

  > **🔧 PATCHER · showrunner-patcher [06-15-26]:** Resolved all E2 audit findings.
  > - 🔴 Public availability response surface is sanitized in `lib/embed/public-scheduling.ts`: `serializePublicSlotDiagnostics` now filters to `slot.available` and serializes only public slot fields via `serializePublicSlot`. It no longer returns native `diagnostics.messages`, unavailable slots, `available` flags, or `reasons`, so customer names, blockout/Google busy details, and internal scheduling config messages stay server-side.
  > - 🟠 Booking error handling is bounded at the public boundary. Known native booking validation/conflict messages are mapped to `EmbedRequestError` statuses (`400`, `404`, or `409`) before leaving `createPublicSchedulingBooking`, while `embedError` now returns raw messages only for `EmbedRequestError`; unexpected exceptions return a generic `Request failed.` 500 body.
  > - 🟡 Honeypot handling now runs before the site/IP `booking_submission` limiter in `app/api/public/v1/bookings/route.ts`: the route parses JSON, discards `companyWebsite` bodies with the same no-op success payload, and only then increments booking quota. The helper remains inside `createPublicSchedulingBooking` as a second guard.
  >
  > Verification: `npx prisma validate`, `npx prisma generate`, `npx tsc --noEmit --pretty false`, touched-file `npx eslint app/api/public/v1/bookings/route.ts lib/embed/gateway.ts lib/embed/public-scheduling.ts --max-warnings=0`, and `git diff --check` pass. No schema changes.
  >
  > **Status: `READY-FOR-CONFIRM`**

  > **✅ VALIDATOR · showrunner-validator [06-15-26]:** Confirmed E2 fixes from PATCHER commit `813d35c` in code.
  > - 🔴 Public diagnostics leak is closed: `serializePublicSlotDiagnostics` filters native diagnostics to `slot.available` and maps through `serializePublicSlot`, so the public `/api/public/v1/availability` response no longer includes `diagnostics.messages`, unavailable slots, `available` booleans, or native `reasons`/`reason.message` strings.
  > - 🟠 Booking error boundary is closed: every current plain `Error` thrown by `nativeSchedulingAdapter.createBooking` is covered by `expectedBookingErrorStatuses` and converted to `EmbedRequestError` (`400`, `404`, or `409`), while `embedError` reflects only `EmbedRequestError` messages and returns generic `Request failed.` for unexpected exceptions.
  > - 🟡 Honeypot ordering is fixed: `POST /api/public/v1/bookings` parses JSON and returns the no-op success payload for `companyWebsite` bodies before calling `publicRateLimitForSite(..., "booking_submission", ...)`; `createPublicSchedulingBooking` keeps the same guard as defense in depth.
  >
  > Verification: `npx prisma validate`, `npx prisma generate`, `npx tsc --noEmit --pretty false`, `npx eslint app/api/public/v1/bookings/route.ts lib/embed/gateway.ts lib/embed/public-scheduling.ts --max-warnings=0`, and `git diff --check` all pass. Working tree was clean before validation; only this roadmap confirmation block/status was edited.
  >
  > **Status: `CONFIRMED`**

  > **🛠 ENGINEER · showrunner-engineer [06-15-26 00:27 CDT]:** READY-FOR-AUDIT for **E3 booking Web Component**.
  > - Added `/embed/v1/booking.js` as a route-served browser script with `Content-Type: application/javascript`, short public cache headers, and `X-Content-Type-Options: nosniff`.
  > - The script registers `<showrunner-booking>` exactly once, attaches Shadow DOM, and renders a complete booking flow: service selector, optional staff selector, date picker, available slot buttons, customer details, optional intake/policy controls, honeypot field, success state, manage link, and calendar link.
  > - Configuration is via attributes: `publishable-key` (or `key`), `api-base`, `service-id`, `service-slug`, `staff-id`, `theme` JSON, and theme token attrs (`accent-color`/`primary-color`, `background-color`, `border-color`, `muted-color`, `text-color`, `radius`). The default API base is the script origin so cross-site embeds call the Showrunner API host, not the customer page host.
  > - E3 consumes only confirmed E2 surfaces: `GET /api/public/v1/services`, `GET /api/public/v1/availability`, and `POST /api/public/v1/bookings` with the publishable key in `X-Showrunner-Key`. It does not call native/admin/private scheduling routes and does not duplicate availability or booking engines.
  > - The component dispatches composed events for host integration: `showrunner:ready`, `showrunner:availability`, `showrunner:booking-created`, and `showrunner:error`.
  > - Scheduling manifest now declares the widget route in `widgetRoutes`.
  >
  > Verification: `npx prisma validate` ✓, `npx tsc --noEmit --pretty false` ✓, `npx eslint app/embed/v1/[asset]/route.ts modules/scheduling/module.ts --max-warnings=0` ✓. Runtime on local Next (`http://127.0.0.1:3000`): fetched `/embed/v1/booking.js` as JavaScript, fetched E2 services with a disposable `scheduling:read/write` key and ACAO, then executed the served script in a DOM-shim smoke test that registered the element, loaded 1 service + 13 slots, rendered Shadow DOM markup, and submitted a booking through the widget path, creating confirmed booking `cmqertg120007b0vioqxww70b` and dispatching `showrunner:booking-created`. Browser plugin verification was attempted but no `iab` browser backend was available, and the repo does not include Playwright/jsdom; the disposable key was revoked after testing.
  >
  > **Status: `READY-FOR-AUDIT`**
...
- Admin views: calendar month/week/day, agenda queue, staff/resource filters, drag-to-reschedule, bulk blockouts, and conflict warnings.
  ...
  > **🛠 ENGINEER · showrunner-worker-2 [06-13-26]:** §2 scheduling waitlist + request-only approval delivered in commit `3c1ec2b` (`Add scheduling waitlist request approvals`) during the temporary lint pause. Added additive scheduling schema/migration for `Service.requestOnly`, `Service.waitlistEnabled`, `BookingWaitlistStatus`, and `BookingWaitlistEntry`; scheduling service setup exposes request-only approval and waitlist toggles. Public booking still uses the existing availability + Serializable booking adapter path; normal services create `CONFIRMED` bookings, request-only services create `PENDING` requests. Waitlist join appears when a selected date has no slots and the service has waitlist enabled, preserving service/date/staff preference/contact/notes/intake/policy data. Appointments admin adds a waitlist desk with promote/decline actions; promotion calls `nativeSchedulingAdapter.createBooking` with `CONFIRMED`, so availability/conflict/resource/staff validation is reused rather than bypassed. Added `getAccessibleBookingWaitlistWhere` to match appointments staff/data-scope scoping, kept request approval on existing `updateBookingStatusAction`, and added lifecycle events for `booking.request.approved`, `booking.waitlist.joined`, `booking.waitlist.promoted`, and `booking.waitlist.declined`.
  >
  > Validation reported: `npx prisma generate`, `npx prisma validate`, `npx tsc --noEmit --pretty false`, focused eslint for touched booking/scheduling/appointments paths, `npm run lint`, `npm run build`, `git diff --cached --check`, and `git show --check --oneline 3c1ec2b`.
  >
  > **Status: `READY-FOR-AUDIT`** (not yet audited)
  >
  > **🔍 LINTER · Claude [06-13-26]:** Audited `3c1ec2b`. The booking-engine reuse is correct and is the part that matters most: promotion goes through `nativeSchedulingAdapter.createBooking({ status: CONFIRMED })` (`modules/appointments/actions.ts`), so the Serializable conflict/resource/staff re-check is reused — no parallel booking path — and request-only services create `PENDING` bookings that **reserve the slot** through the existing conflict query (`status: { not: CANCELED }`, `lib/scheduling/native.ts:465-487`), so approving a request via `updateBookingStatus` (which does not re-check availability) cannot double-book. Admin actions are `appointments:manage`-gated and record-scoped via the new `getAccessibleBookingWaitlistWhere`, which reuses the shared `getAccessibleModuleWhere("appointments", …)` engine (no IDOR). New lifecycle events are fully wired to `AutomationTrigger` values through the event slice (`modules/appointments/events.ts:23-46`) — not dead enum values — and the request-approved event fires on the real PENDING→CONFIRMED edge (`lib/bookings/status.ts:26-35`). Schema is additive with correct FK `onDelete` + indexes; migration matches. Findings:
  > - 🟠 HIGH — **`joinPublicWaitlistAction` has no honeypot and no per-IP rate limit** (`app/book/actions.ts`). It is an unauthenticated POST that upserts a `Client` row and inserts a `BookingWaitlistEntry` for any future date with no slot/availability constraint, so a bot can flood the waitlist desk and mass-create/mutate client records — exactly the abuse vector the §8 anti-abuse pass closed for `createPublicFormSubmissionAction`/`createPublicTestimonialAction` via `publicRateLimitMessage` + honeypot. Add the same honeypot + `publicRateLimitMessage(...)` gate before the client/entry writes. **Sidebar (not tagged for this audit):** the sibling `createPublicBookingAction` (`app/book/actions.ts:59-89`) *also* has no honeypot/throttle, so the entire public `/book` write surface is unthrottled; the waitlist inherits and widens that gap (it isn't bounded by real slot availability). Worth a tracked cross-cutting fix.
  > - 🟡 MEDIUM — **Public client upsert overwrites an existing client's name/phone.** `joinPublicWaitlistAction`'s `tx.client.upsert` runs `update: { name, phone }` (`app/book/actions.ts`), so a join using a known client's email rewrites that client's stored name/phone from anonymous input — the overwrite the §7/§8 audit fixed for forms/testimonials (create-only / fill-blanks). This mirrors the existing `createBooking` upsert (`lib/scheduling/native.ts:489-501`), so it is a booking-module-wide deviation from the create-only rule rather than a waitlist-only regression; align both paths.
  > - 🟡 MEDIUM — **Promotion is not atomic and not guarded against double-promotion.** `promoteWaitlistEntryAction` reads the entry filtered `status: WAITING`, creates the booking in a separate transaction, then *unconditionally* updates the entry to `PROMOTED` (`modules/appointments/actions.ts`). Two concurrent promotions — or a retry after a crash between `createBooking` and the status flip — can both pass the WAITING read and create duplicate bookings from one entry. Low real-world likelihood (admin-initiated, low concurrency), but the project's established pattern (reminder claim, coupon redemption) uses a conditional `updateMany({ where: { id, status: WAITING } })` claim; do the same (ideally claim → create → release-on-failure).
  > - 🟢 LOW — `getAccessibleBookingWaitlistWhere` reuses the appointments ownership scope, which keys on `staffId`. But a waitlist entry's `staffId` is an optional customer *preference*, not an owning staffer, so entries with no preference (`staffId: null`) are invisible to data-scope-restricted staff and visible only to unscoped roles. Confirm restricted staff aren't meant to triage unassigned waitlist requests.
  >
  > **Status: `READY-FOR-REVIEW`**
  >
  > **🔧 PATCHER · claude-patcher [06-13-26]:** No reviewer in this rotation → straight to PATCHER. Committed `6872aef`. All three flagged findings fixed, plus the cross-cutting sidebar:
  > - 🟠 HIGH (incl. the `createPublicBookingAction` sidebar) — added the established honeypot (`companyWebsite`) + per-IP `publicRateLimitMessage` gate to BOTH public `/book` write actions (`app/book/actions.ts:60-67,135-142`): `createPublicBookingAction` uses scope `booking_submission` (6/10min), `joinPublicWaitlistAction` uses `waitlist_join` (6/10min); honeypot returns a silent success and the rate-limit returns the friendly error, before any client/booking/entry write. Added the hidden honeypot field to the shared review form (`app/book/booking-flow.tsx:410-417`). The whole public booking surface is now throttled, closing both the waitlist gap and the sibling booking gap.
  > - 🟡 MEDIUM (client-upsert overwrite) — both public paths now go through a new shared `upsertPublicClient` (`lib/clients/public-client.ts`) that is create-or-fill-blank: it NEVER overwrites an existing client's stored name/phone from anonymous input (only fills empty fields), matching the §7/§8 create-only rule. Wired into the native booking path (`lib/scheduling/native.ts:490-495`) and the waitlist join (`app/book/actions.ts:183-188`), so the booking-module-wide deviation is fixed at both sites with one reused helper (Prime Directive).
  > - 🟡 MEDIUM (non-atomic promotion) — `promoteWaitlistEntryAction` now atomically claims the entry with a conditional `updateMany({ where:{id,status:WAITING}, data:{status:PROMOTED} })` BEFORE `createBooking` and bails if `count !== 1`; on booking-create failure it releases the claim back to WAITING (`modules/appointments/actions.ts:239-266`). Two concurrent promotions (or a crash-retry) can no longer create duplicate bookings from one entry — the established claim → create → release-on-failure pattern.
  > - 🟢 LOW (waitlist scope on staff *preference*) — left as-is; this is a data-scope intent question (whether restricted staff should triage unassigned waitlist requests), consistent with the chunk-4 record-scope decision. Carried as a confirm-with-note for the validator/@user, not a defect to patch here.
  >
  > Verification: `npx tsc --noEmit --pretty false` clean; `npx eslint` clean on all touched files.
  >
  > **Status: `READY-FOR-CONFIRM`**

  > **✅ VALIDATOR · Claude [06-14-26]:** Confirmed in code, all four findings hold.
  > - 🟠 HIGH (honeypot + rate limit) — `app/book/actions.ts`: `companyWebsite` honeypot defined (`:17`); `createPublicBookingAction` checks honeypot (`:64-66`, silent `{ok:true}`) then `publicRateLimitMessage("booking_submission", {limit:6, windowMinutes:10})` (`:68-71`), both before the booking write (`:91`); `joinPublicWaitlistAction` checks honeypot (`:148-150`) then `publicRateLimitMessage("waitlist_join", {limit:6, windowMinutes:10})` (`:152-155`), both before the `$transaction` (`:205`). Hidden `companyWebsite` input added to the shared review form (`app/book/booking-flow.tsx:410-417`, `aria-hidden`, `display:none`, `tabIndex={-1}`).
  > - 🟡 MEDIUM (create-or-fill-blank client upsert) — `lib/clients/public-client.ts:22-52` `upsertPublicClient` only sets `name`/`phone` on update `if (!existing.name && name)` / `if (!existing.phone && phone)` — never overwrites a populated field. Wired into native booking (`lib/scheduling/native.ts:490,500`) and waitlist join (`app/book/actions.ts:206`).
  > - 🟡 MEDIUM (atomic promotion claim) — `modules/appointments/actions.ts` `promoteWaitlistEntryAction` (from `:201`): conditional `updateMany({where:{id,status:WAITING}, data:{status:PROMOTED}})` (`:242-245`), bails on `count!==1` (`:246-248`), `createBooking` in try (`:252-263`), catch releases claim back to `WAITING` on failure (`:264-274`).
  > - 🟢 LOW (waitlist-scope-on-preference) — left as confirm-with-note per the patcher, consistent with the chunk-4 data-scope decision; no further action needed.
  >
  > 🟢 fwd note (not a defect): `upsertPublicClient` falls back to `name: name || email` for brand-new clients with no name supplied — pre-existing-style behavior, only matters for display polish.
  >
  > **Status: `CONFIRMED`**
...
- Order management: orders, statuses, fulfillment, refunds, notes, invoices/receipts, customer history, and admin notifications. Order/status/customer-link data foundation is audited 06-06-26; fulfillment UI, refunds, receipts, and notifications remain pending.
  ...
  > **🛠 ENGINEER · showrunner-worker-4 [06-13-26]:** §3/§5 commerce fulfillment exports + print/photo lab workflow hooks delivered in commit `8f7ec27` (`Add fulfillment export handoff workflow`). Added additive order-level fulfillment export and print/photo lab handoff metadata with migration `20260613151000_fulfillment_exports_print_lab`; added `/admin/modules/products/fulfillment-export` CSV export with one row per paid physical order item; included order/customer/item data, export/handoff metadata, and derived lab/gallery/media metadata from existing `Product.attributes` keys, resolving referenced gallery/media labels where records exist. Added audited admin actions to mark a paid physical order exported and record print lab handoff for paid/fulfilled physical orders. Products admin now exposes fulfillment CSV download, selected-order export batch, and print-lab handoff controls. Checkout totals/payment settlement were left untouched, and fulfillment status transition remains through the dedicated fulfillment action from `baf2296`.
  >
  > Validation reported: `npx prisma generate`, `npx prisma validate`, focused eslint for touched products/export route files, `npm run lint`, `npx tsc --noEmit`, `npm run build`, `git diff --cached --check`, and `git show --check --stat --oneline HEAD`.
  >
  > **Status: `READY-FOR-AUDIT`** (not yet audited)
  >
  > **🔍 LINTER · Claude [06-13-26]:** Audited `8f7ec27`. Clean, pattern-consistent commerce work. The export route is `requireAdmin("products:manage")`-gated, fully tenancy-scoped (orders by `siteId`; galleries via `gallery.siteId`, gallery items, and media all re-scoped to `settings.siteId` — `modules/products/api/fulfillment-export.ts:60-118`), audit-logged, and — importantly for a spreadsheet export — runs through the shared `csvDocument(..., { preventFormulaInjection: true })` helper (`lib/api/csv.ts:9`), so `=,+,-,@`-prefixed cells are neutralized (no CSV/formula injection from attacker-controllable order/customer/product text). The two state actions (`markCommerceOrderFulfillmentExportedAction`, `recordCommerceOrderPrintLabHandoffAction`) match the established order-action pattern: admin-gated, `findFirst({ id, siteId })` (no IDOR), status-guarded (PAID for export, PAID/FULFILLED for handoff), physical-item-guarded, audit-logged, and the success `redirect()` sits outside the try/catch (the §3 redirect-throw trap is avoided). Migration matches schema (additive, all-defaulted columns + two `siteId`-prefixed indexes), and the route is a one-line re-export honoring the co-located API convention. Findings:
  > - 🟢 LOW — The CSV export emits **every** paid physical order on every download (no "un-exported only" filter, no date/batch scoping — `fulfillment-export.ts:60-83`), so repeated pulls re-include already-handed-off orders. The `Fulfillment Exported At`/`Batch` columns are in the output so a lab can dedupe, but a server-side "since last export" filter would be safer at volume.
  > - 🟢 LOW — Downloading the CSV does **not** set `fulfillmentExportedAt`; only the separate "Mark exported" button does, so the export-tracking flag can drift from what was actually downloaded. Appears intentional (admin controls batch labeling) — confirm.
  > - 🟢 LOW — Order admin actions scope by `siteId` + `products:manage` only, not the §14b record-level `getAccessible*Where`. This is consistent with the pre-existing commerce order actions in the same file (commerce orders are site-level, not per-staff-owned), so not a regression introduced here.
  >
  > **Status: `READY-FOR-REVIEW`**
  >
  > **🔧 PATCHER · claude-patcher [06-13-26]:** No reviewer in this rotation → straight to PATCHER. Committed `a5ffc52`. All three are 🟢 LOW; handled as the auditor framed them:
  > - 🟢 (re-exports every paid order each pull) — addressed with a real improvement: opt-in `?unexportedOnly=1` query scoping on the export route adds `fulfillmentExportedAt: null` to the where so a lab can pull only un-exported deltas (`modules/products/api/fulfillment-export.ts:57-71`); the default full-snapshot behavior is unchanged. Added an "Export new orders only" admin button alongside the existing one (`modules/products/page.tsx:623-632`), and the filter flag is recorded in the export audit entry.
  > - 🟢 (download doesn't set `fulfillmentExportedAt`) — confirmed intentional: the admin controls batch labeling via the explicit "Mark exported" action, and the new delta export keys off that same flag, so download-without-flag is by design. Left as-is (confirm-with-note).
  > - 🟢 (order actions site-level, not §14b record-scoped) — confirmed not a regression: identical to the pre-existing commerce order actions in the same file (commerce orders are site-level, not per-staff-owned). Left as-is (confirm-with-note); a future change to record-scope ALL commerce order actions would be a cross-cutting §14b item, not this delivery.
  >
  > Verification: `npx tsc --noEmit --pretty false` clean; `npx eslint` clean on both touched files.
  >
  > **Status: `READY-FOR-CONFIRM`**

  > **✅ VALIDATOR · Claude [06-14-26]:** Confirmed in code, all three findings hold.
  > - 🟢 (opt-in delta export) — `modules/products/api/fulfillment-export.ts:63` reads `unexportedOnly` from the query string and `:68` conditionally spreads `fulfillmentExportedAt: null` into the `where`; default (no param) behavior unchanged.
  > - 🟢 ("Export new orders only" button) — `modules/products/page.tsx:624-627` keeps the existing full-export link; `:628-631` adds the new button linking to `?unexportedOnly=1`.
  > - 🟢 (filter recorded in audit) — `fulfillment-export.ts:208-221` `recordAuditLog` metadata includes `unexportedOnly` alongside `orderCount`/`rowCount`.
  > Re-verified no regression: still `requireAdmin("products:manage")`-gated (`:58`), tenancy-scoped by `siteId` across orders/galleries/gallery items/media (`:66,101,107,120`), and still uses `csvDocument(..., { preventFormulaInjection: true })` (`:223`).
  >
  > **Status: `CONFIRMED`**
...
- Automations: create/link client from inquiry or client-targeted form submission plus form-submission email notification are audited with fixes applied as of 06-07-26; create booking request, add tag, create invoice, and unlock gallery remain pending.
  ...
  > **🛠 ENGINEER · showrunner-worker-1 [06-13-26]:** §7 Forms file-upload fields delivered in commit `fc0e5f5` (`Add form file upload fields`). Added additive `FormFieldType.FILE` enum value + migration `20260613155500_form_file_upload_fields`; owner file MIME allowlist and max upload size config live in existing `FormField.validationRules`. Public forms render multipart file inputs with configured `accept`, preserve conditional/multi-step behavior, and add client-side required/MIME/size validation. Server submission remains authoritative: honeypot and rate limit run before upload, visibility is recomputed server-side, hidden-by-condition file fields are skipped, and required/MIME/size validation runs before storage. Reuses existing `MediaAsset`/R2 storage primitives by extending `uploadMedia` with an opt-in private-file validation mode while keeping existing media-module image upload behavior image-only by default. Submission JSON stores bounded file metadata (`assetId`, filename, MIME, size), not file contents. Added gated admin download route `/admin/modules/forms/submissions/[submissionId]/files/[assetId]` requiring `forms:manage`, form-submission data scope, asset-reference verification, and private media serving through `mediaDeliveryResponse`; admin submission detail displays file download links with MIME/size metadata.
  >
  > Validation reported: `npx prisma validate`, `npx prisma generate`, `npx tsc --noEmit --pretty false`, focused eslint for touched Forms/media files, `npm run lint`, `npm run build`, `git diff --check` for intended paths, `git diff --cached --check`, and `git show --check --oneline --stat HEAD`.
  >
  > **Status: `READY-FOR-AUDIT`** (not yet audited)
  >
  > **🔍 LINTER · Claude [06-13-26]:** Audited `fc0e5f5`. Strong, security-conscious build of an unauthenticated upload surface. Verified the defense-in-depth chain: on public submit, honeypot → `publicRateLimitMessage("form_submission")` → server-recomputed visibility all run before any upload (`modules/forms/actions.ts:791-852`), hidden-by-condition FILE fields are skipped (no upload), and `validateUploadedFile` checks declared MIME + size before `uploadMedia` re-validates by **magic bytes** (`detectUploadMimeType` adds `%PDF-` and rejects declared/detected mismatch — `lib/media.ts:171-205`). The MIME allowlist is constrained to images + PDF with SVG deliberately excluded (no stored-XSS vector), and a hard 25 MB ceiling caps size. Files are stored `isPrivate: true`, tenancy-scoped (`settings.siteId`, `folder: forms/<id>`), and submission JSON keeps only bounded metadata (assetId/filename/mime/size), never contents. The admin download route is the right shape: `forms:manage`-gated, module-enabled-gated, **record-scoped** via `getAccessibleFormSubmissionWhere` (no IDOR), verifies the submission actually references the asset before serving, requires `asset.isPrivate`, and serves through `mediaDeliveryResponse({ download: true })` which forces `Content-Disposition: attachment` (`lib/media.ts:743-744`) so a PDF/file can't render inline in the admin's session. Filenames are `safeFilename`-sanitized at download and React-escaped in the UI. Findings:
  > - 🟡 MEDIUM — **Orphaned private uploads on failed/abandoned submissions.** File fields upload to R2 (creating a `MediaAsset` row) the moment the field loop reaches them (`modules/forms/actions.ts:898-955`), but the `FormSubmission` is only persisted after *all* fields validate. Any later-field validation failure — a second file field failing MIME, or a downstream required text field left blank — leaves the already-uploaded asset(s) as orphans: unreferenced by any submission (hence unreachable via the asset-reference-checked download route) but still consuming private R2 storage with no cleanup path. On a public endpoint, repeated failed/abandoned multi-field submissions accumulate. Consider deferring uploads until all non-file fields validate, or deleting uploaded assets on the error/redirect path.
  > - 🟢 LOW — No per-submission aggregate upload cap: a form with N FILE fields permits N × maxSize of uploads per accepted submission before the per-IP `form_submission` window refreshes. Bounded by admin form design + the rate limit (not an unbounded-write hole), but a multi-file form has a larger per-request cost than the single-field default contemplates.
  >
  > **Status: `READY-FOR-REVIEW`**
  >
  > **🔧 PATCHER · claude-patcher [06-13-26]:** No reviewer in this rotation → straight to PATCHER. Committed `18cc1e4`. Both findings closed:
  > - 🟡 MEDIUM (orphaned uploads) — the public submission now records every uploaded asset id (`uploadedAssetIds`) and wraps the field loop + client/submission persistence in a try/catch. A `submissionPersisted` flag flips to true immediately after `formSubmission.create` (`modules/forms/actions.ts`); on any throw BEFORE that point — a later FILE/text field validation `redirect`, a downstream required-field failure, or a create error — the catch deletes every already-uploaded asset (best-effort) and re-raises so the original redirect/error still flows. A failure AFTER persistence (signatures/email/events) does NOT delete, since the saved submission now references the files. To actually remove storage objects I implemented the missing R2 `delete` (`DeleteObjectCommand`) on the media adapter and a reusable `deleteMediaAsset(assetId, siteId)` helper (`lib/media.ts`) that deletes the stored object + the `MediaAsset` row (variants cascade), siteId-scoped and never-throws (cleanup can't mask the original failure).
  > - 🟢 LOW (no aggregate cap) — added a per-submission aggregate upload cap (`maxSubmissionUploadBytes = 25MB`): cumulative file bytes are summed across FILE fields and a submission that exceeds the cap is rejected with a friendly message (the partial uploads are then cleaned up by the same orphan path).
  >
  > Verification: `npx tsc --noEmit --pretty false` clean; `npx eslint modules/forms/actions.ts lib/media.ts` clean. (Note: the new `deleteMediaAsset` helper is general-purpose and can later back a real hard-delete in the media module, which currently only soft-deletes.)
  >
  > **Status: `READY-FOR-CONFIRM`**

  > **✅ VALIDATOR · Claude [06-14-26]:** Confirmed in code, both findings hold.
  > - 🟡 MEDIUM (orphaned uploads) — `modules/forms/actions.ts`: `uploadedAssetIds`/`uploadedBytesTotal` tracked (`:805-806`), pushed/incremented per FILE upload (`:955-956`); field-processing + `formSubmission.create` wrapped in `try` (`:850`) with `catch` at `:1145`; `submissionPersisted = true` set immediately after `formSubmission.create` (`:1059`), before signature/email/event side effects (~`:1060-1144`), so post-persistence failures correctly skip cleanup. Catch block (`:1145-1152`) deletes every uploaded asset via `deleteMediaAsset` when `!submissionPersisted`, then re-throws (preserves `NEXT_REDIRECT`).
  > - `lib/media.ts`: `DeleteObjectCommand` import (`:4`), `r2Adapter.delete` implemented (~`:275`), `deleteMediaAsset(assetId, siteId)` (~`:491`) is `siteId`-scoped, never throws (wrapped in try/catch), deletes storage object + `MediaAsset` row.
  > - 🟢 LOW (aggregate cap) — `maxSubmissionUploadBytes = 25 * 1024 * 1024` (`:42`); cumulative bytes checked at `:957-959`, rejection routes through the same orphan-cleanup catch path.
  > Sanity re-checks: magic-byte MIME validation intact (`lib/media.ts:164-177`, incl. `%PDF-`); `image/svg+xml` still excluded from both allowlists (`:13-19`); form file uploads still `isPrivate: true` (`modules/forms/actions.ts:938`).
  >
  > **Status: `CONFIRMED`**
...
> **🛠 ENGINEER · showrunner-worker-3 [06-13-26]:** §10 Communications delivery-status hardening delivered in commit `64a8d1c` (`Harden email delivery status handling`) during the temporary lint pause. Provider webhook handling now normalizes common provider payload keys/event aliases before recording events; provider event handling maps delivered/delayed/bounced/complained/unsubscribed into `EmailOutbox` status/failure visibility plus suppression/subscriber updates. Communications outbox admin now shows latest provider event + last error and offers manual resend for non-active rows, reusing the existing EmailOutbox queue path and suppression checks. Scope stayed on `lib/email/provider-events.ts`, `modules/communications/api/email-provider-events.ts`, `modules/communications/actions.ts`, and `modules/communications/page.tsx`; no parallel message log/store or send engine.
>
> Validation reported: focused `npx eslint` on the four touched §10 files, `git show --check --oneline --stat 64a8d1c`, `npm run lint`, `npx tsc --noEmit --pretty false`, and `npm run build`.
>
> **Status: `READY-FOR-AUDIT`** (not yet audited)
>
> **🔍 LINTER · Claude [06-13-26]:** Audited `64a8d1c`. Solid, well-scoped hardening that genuinely reuses the existing pipeline — no parallel send engine or message store. Verified: the provider webhook stays secret-gated (`EMAIL_PROVIDER_WEBHOOK_SECRET`, auth unchanged); `normalizeProviderEventType` maps common provider aliases (bounce/complaint/deferred/open/click/unsubscribe) and informational types (ACCEPTED/OPENED/CLICKED) fall through with no status mutation; `recordProviderEvent` dedups on `eventKey` via P2002; and `suppressRecipient` centralizes the subscriber + suppression writes (no duplication). Crucially, marking a bounce/complaint `EmailOutboxStatus.FAILED` is safe — the worker's `claimRows` only re-selects `QUEUED` (due) or stale `SENDING` rows (`lib/email/process.ts:28-41`), never `FAILED`, so there is no resend loop; and the manual resend re-checks suppression before requeuing (`modules/communications/actions.ts` `outboxSuppressionReason`), creating a `SUPPRESSED` row instead of sending to a suppressed address. Admin display is React-escaped (no XSS on `lastError`/provider labels). Findings:
> - 🟢 LOW — Out-of-order/duplicate events can mask a failure in the outbox *status*. A late or duplicate `DELIVERED` after a `BOUNCED`/`COMPLAINED` resets `status` to `SENT` and clears `lastError` (`lib/email/provider-events.ts:89-95`), overwriting the FAILED state shown in the admin table. The `bouncedAt`/`complainedAt` timestamps and the persisted `SuppressionListEntry` survive, so there is no resend risk — only the at-a-glance status can be misleading. Acceptable as best-effort; if precise terminal state matters, ignore `DELIVERED` once a bounce/complaint timestamp is set.
> - 🟢 LOW — For an event whose `providerMessageId` matches no outbox row, `siteId` falls back to `getCurrentSiteId()` (`lib/email/provider-events.ts:68`). In a provider-webhook request there is no meaningful "current site", so an orphan `EmailProviderEvent` is attributed to whatever the host resolves to (typically the default site). These rows are unmatched/informational and `recordProviderEvent` returns at `:87` without touching any outbox, so impact is limited to event-log attribution.
> - 🟢 LOW — `canResendStatus` allows resending a `SENT` (delivered) row (`modules/communications/page.tsx`), so an admin can intentionally trigger a duplicate delivery. This is an admin-initiated manual action and suppression still gates truly-bad addresses, so it is a capability rather than a defect — flagging only so REVIEW can confirm intent.
>
> **Status: `READY-FOR-REVIEW`**
>
> **🔧 PATCHER · claude-patcher [06-13-26]:** No reviewer in this rotation → straight to PATCHER. Committed `8a1bd67`. All three are 🟢 LOW; handled as the auditor framed them:
> - 🟢 (out-of-order DELIVERED masks a failure) — fixed: the DELIVERED branch now checks `outbox.bouncedAt || outbox.complainedAt` and, when a terminal failure is already recorded, writes only `deliveredAt` (for the event log) WITHOUT resetting `status` to SENT or clearing `lastError` (`lib/email/provider-events.ts:89-101`). The admin table keeps showing the real FAILED outcome even if a late/duplicate DELIVERED arrives; suppression was already persisted, so there was never a resend risk.
> - 🟢 (orphan-event `siteId` falls back to `getCurrentSiteId()`) — confirmed informational only: an unmatched event records an `EmailProviderEvent` and returns at `:87` without touching any outbox, so impact is limited to event-log attribution. Left as-is (confirm-with-note); a provider-webhook request has no meaningful "current site" to resolve better without a per-provider site mapping.
> - 🟢 (`canResendStatus` allows resending a SENT row) — confirmed intentional: an admin-initiated manual resend, and the resend path still re-checks suppression before requeuing (a suppressed address yields a SUPPRESSED row, not a send). Capability, not a defect — left as-is (confirm-with-note).
>
> Verification: `npx tsc --noEmit --pretty false` clean; `npx eslint lib/email/provider-events.ts` clean.
>
> **Status: `READY-FOR-CONFIRM`**

> **✅ VALIDATOR · Claude [06-14-26]:** Confirmed in code.
> - 🟢 (out-of-order DELIVERED-after-bounce) — `lib/email/provider-events.ts:89-101`: `hasTerminalFailure = Boolean(outbox.bouncedAt || outbox.complainedAt)`; if true, the update writes only `{ deliveredAt: now }` (status/lastError untouched); otherwise behaves as before (`deliveredAt`, clears `lastError`, sets `SENT`).
> - 🟢 (orphan-event siteId fallback + early return) — unchanged, `provider-events.ts:68,87`. Confirmed informational-only, no regression.
> - 🟢 (worker never re-selects FAILED) — `lib/email/process.ts:26-53` `claimRows` only selects `QUEUED` (due) or stale `SENDING`; no `FAILED` branch.
> - 🟢 (manual resend re-checks suppression) — `modules/communications/actions.ts:686-690` calls `outboxSuppressionReason(...)` before resend; `:716` sets `SUPPRESSED` vs `QUEUED` accordingly.
>
> **Status: `CONFIRMED`**
...
- Public accept/pay links, hosted Stripe checkout handoff, partial payments, and PDF rendering are now built (🔍 AUDITED — see depth block below). Recurring invoices, partial-refund handling, and contract signing/versioning remain pending.
  ...
  > **Status: `CONFIRMED`**
  >
  > **🔧 PATCHER · Claude [06-14-26]:** Addressed the 🟢 forward item ("partial-refund ledger + a refund path for rejected-but-captured payments"). The partial-refund ledger (`BillingPayment.refundedCents`, `refundBillingPaymentAction`, `rollbackBillingRefundReservation`) already existed — only the rejected-but-captured refund path was missing.
  > - Added `isRejectedCapturedPayment` + `REJECTED_OVERPAYMENT_REASON` to `lib/billing/payments.ts`: true when `status===FAILED`, `refundedCents===0`, `externalPaymentId` is set, and `rawSummary.rejectedReason==="payment_would_exceed_remaining_balance"` — i.e. exactly the durable-FAILED state the `0c9b319` settle patch leaves behind when Stripe already captured funds for an over-credit attempt.
  > - `refundBillingPaymentAction` (`modules/billing/actions.ts`) now also accepts payments in that state: eligibility check OR's in `isRejectedCapturedPayment(payment)`, `remainingCents` is the full `amountCents` for that case, the reservation `updateMany` matches `status: FAILED` (instead of PAID/AUTHORIZED) when rejected-but-captured, and the post-refund status flip also matches `FAILED→REFUNDED` for that path. The existing gateway-refund call, rollback-on-failure, and `billing.payment_refunded` audit log are reused unchanged.
  > - `refundablePaymentCents` (`modules/billing/page.tsx`) now returns `payment.amountCents` for a rejected-but-captured row (in addition to the existing PAID/AUTHORIZED − refunded calc), so the per-payment refund form in the billing document view appears for these rows.
  > - `npx tsc --noEmit` green.
  >
  > **Status: `READY-FOR-CONFIRM`**

  > **✅ VALIDATOR · Claude [06-14-26]:** Confirmed in code.
  > - `lib/billing/payments.ts:154-173` — `REJECTED_OVERPAYMENT_REASON = "payment_would_exceed_remaining_balance"` (`:154`); `isRejectedCapturedPayment` returns true only when `status===FAILED` (`:162`), `refundedCents===0` (`:163`), `externalPaymentId` is set (`:164`), and `rawSummary.rejectedReason === REJECTED_OVERPAYMENT_REASON` (`:167-172`) — exactly the state left by the `0c9b319` over-credit rejection write (`:87-101`, which still hardcodes the matching literal string at `:95`).
  > - `modules/billing/actions.ts:660-709` — `rejectedButCaptured = isRejectedCapturedPayment(payment)` (`:660`); eligibility check ORs it in alongside PAID/AUTHORIZED (`:661`); `remainingCents` is the full `amountCents` for that case (`:665`); the reservation `updateMany` matches `status: FAILED` (vs. PAID/AUTHORIZED) when `rejectedButCaptured` (`:675`); the terminal status flip also matches `FAILED→REFUNDED` for that path (`:706`); gateway refund call, rollback-on-failure (`:686-700`), and `billing.payment_refunded` audit log (`:715-728`) are unchanged/reused.
  > - `modules/billing/page.tsx:61-75` — `refundablePaymentCents` returns `amountCents - refundedCents` for PAID/AUTHORIZED (`:68-69`), and for a rejected-but-captured row returns the full `amountCents` (`:71-73`), so the refund form (`:531,548`) renders for those rows.
  > - `npx tsc --noEmit --pretty false` clean on the full tree.
  >
  > **Status: `CONFIRMED`**
...
- Carry over from chunk-3: add a regression test/lint rule asserting every admin mutation passes an explicit permission (default-deny can't silently regress); pass the accessible-where into the `updateClient` mutation for defense-in-depth.
  ...
  > **Status: `CONFIRMED`**
  >
  > **🔧 PATCHER · Claude [06-14-26]:** Patched the two 🟢 forward items from the prior VALIDATOR pass.
  > - Migration `20260614120000_staff_admin_link_collision_fix` replaces `sync_staff_member_admin_user_id` (BEFORE INSERT/UPDATE OF email on `StaffMember`) and `sync_admin_user_staff_profiles` (AFTER INSERT/UPDATE OF email on `AdminUser`). The first now leaves `NEW."adminUserId"` NULL if another `StaffMember` row in the same site already holds that `adminUserId`; the second updates at most one `StaffMember` per site (`DISTINCT ON ("siteId") ... ORDER BY "siteId", "createdAt" ASC, "id" ASC`). Two staff profiles in one site sharing an email that matches one `AdminUser` can no longer trigger a `StaffMember_siteId_adminUserId_key` violation on insert/update.
  > - Added a manual link UI: `linkStaffMemberAdminUserAction` (`modules/scheduling/actions.ts`, gated on `users:manage`) lets an owner directly set/clear a `StaffMember.adminUserId` (atomically unlinking any other staff in the site currently holding that admin user first, in a `$transaction`). Rendered in the staff roster (`modules/scheduling/components/staff-panel.tsx`) as a "Linked admin account" dropdown, only for users with `users:manage`; `modules/scheduling/page.tsx` loads the site's `AdminUser` list only when that permission is present.
  > - `npx tsc --noEmit` green.
  >
  > **Status: `READY-FOR-CONFIRM`**

  > **✅ VALIDATOR · Claude [06-14-26]:** Confirmed in code.
  > - Migration `prisma/migrations/20260614120000_staff_admin_link_collision_fix/migration.sql`: `sync_staff_member_admin_user_id` (`:5-39`) sets `NEW."adminUserId"` to NULL whenever another `StaffMember` row in the same site already holds the matched `adminUserId` (`:26-35`); `sync_admin_user_staff_profiles` (`:41-63`) first clears any staff whose email no longer matches (`:44-47`), then links exactly one `StaffMember` per site via `DISTINCT ON ("siteId") ... ORDER BY "siteId","createdAt" ASC,"id" ASC` (`:49-59`). Collision-safe by construction — no path can produce two `StaffMember` rows in one site with the same `adminUserId`.
  > - `linkStaffMemberAdminUserAction` (`modules/scheduling/actions.ts:293-335`): `requireAdmin("users:manage")` gate (`:294`); validates `staffId` belongs to the current site (`:305-311`) and, if set, that `adminUserId` exists (`:313-318`); inside one `$transaction`, clears any other staff in the site currently holding that `adminUserId` (`:321-326`) before setting it on the target staff member (`:328-331`).
  > - UI: `staff-panel.tsx:111-134` renders the "Linked admin account" `<select>` (defaulting to `member.adminUserId`) only when `canLinkStaffAccounts` is true; `page.tsx:33,56` computes that flag from `hasAdminPermission(adminUser, "users:manage")` and only queries `prisma.adminUser.findMany` when the permission is present. `adminUser.findUnique`/`findMany` are unscoped by site, matching the existing global-`AdminUser` pattern used by `modules/users/page.tsx` — not a new IDOR.
  > - `npx tsc --noEmit --pretty false` clean on the full tree.
  >
  > **Status: `CONFIRMED`**
...
- Visual email template library/editor and automation template picker. Spike `usewaypoint/email-builder-js`; if compatible, add the builder on top of `MessageTemplate`, seed workflow templates, and wire automation `SEND_EMAIL` rules to pick a template from the library.
  ...
  > **Status: `READY-FOR-AUDIT`**
  >
  > **🔍 LINTER · Claude [06-14-26]:** Audited the MJML renderer switch, `MessageTemplateVersion`/clone/restore, and sample-event preview. The MJML pipeline itself is sound: `safeUrl`'s `{{token}}` passthrough (`lib/email-builder/render.ts:28-34`) is safe because `renderEmailTemplate` substitutes tokens and then re-sanitizes the final output (`lib/email/render.ts:79`), so a malicious resolved URL scheme is stripped by `sanitizeEmailHtml`'s scheme allowlist — confirmed safe, not a defect. The prior 🟢 LOW on free-text `fontFamily` injection is resolved: `fontFamilySchema` is now a closed enum with `.catch()` (`document.ts:17-26`). Findings, priority order:
  >
  > - 🟠 SIGNIFICANT — **`updateMessageTemplateBuilderAction` has no system-template (`key`) guard, and the gap is wider than the Status Index tracks.** `modules/communications/actions.ts:372-448` does a bare `findFirst({ id, siteId, channel: EMAIL })` with no `key` check, unlike `updateMessageTemplateStatusAction` ("System templates are read-only from this catalog", `actions.ts:308-310`) and `updateBookingTemplateSettingsAction`'s `bookingTemplateKeySet.has(template.key)` allowlist (`actions.ts:340`). The "Visual email builder" panel (`modules/communications/page.tsx:398-445`) iterates `emailTemplates` (page.tsx:250) — every EMAIL template, system-keyed or not. Per `prisma/seed.ts`, that's at least 13 system-keyed EMAIL templates (8 `booking.*` plus `form.submitted.admin`, `billing.document.customer`, `order.checkout.customer`, `order.receipt.customer`, `cart.recovery.customer`), all exposed to this unguarded full rewrite (subject/previewText/textBody/htmlBody/builderJson/senderIdentityId). The Status Index (roadmap.md:295) marks "core" ✅ CONFIRMED with "only open item: clone/restore inherit the §14 communications enforcement gap" — but `updateMessageTemplateBuilderAction` *is* the core save path and has the identical gap, for a larger template set than the 8 booking keys that even have a dedicated editor. The 5 non-booking system templates have no dedicated editor at all, so the visual builder may be their *only* intended admin-edit surface — that's a decision worth making deliberately, not leaving as an unguarded side effect.
  >
  > - 🟠 SIGNIFICANT — **Once `builderJson` is set on a system template via the path above, `htmlBody` becomes silently dead for that template, with no indication in the admin UI.** `renderEmailTemplate` (`lib/email/render.ts:68-74`) renders from `builderJson` whenever present, ignoring `htmlBody` entirely. `updateBookingTemplateSettingsAction` (`actions.ts:320-370`) still exposes an "HTML body" textarea (`page.tsx:346-348`) for the same 8 booking templates with no warning that it becomes inert once a builder document exists — that save succeeds (`?saved=booking-template`) but has zero effect on the sent email. This reproduces the "two systems disagree silently" failure mode the original §10 audit closed.
  >
  > - 🟡 MINOR — **`hasBuilderJson` predicate is duplicated.** The identical object/array/key-count check is implemented twice: inline in `lib/email/render.ts:68-71` and as a named helper in `modules/communications/page.tsx:158-160`. `page.tsx` already imports from `@/lib/email/render` (line 12), so exporting one shared predicate from there would satisfy "never duplicate logic that already exists elsewhere."
  >
  > - 🟡 MINOR — `restoreMessageTemplateVersionAction` (`actions.ts:513-578`) has the same missing-`key`-guard issue as the builder action above (restores `builderJson`/`isActive` wholesale for any template regardless of `key`), and does not re-run `assertTemplateTokens` after restoring — a version captured under an older `requiredTokens`/allowlist could be restored without re-validation.
  >
  > - 🟡 MINOR — `cloneMessageTemplateAction`'s post-create snapshot call (`actions.ts:507`) — `snapshotMessageTemplate({ ...cloned, builderRenderer: cloned.builderRenderer }, "Initial cloned version")` — spreads `cloned` and re-assigns a field it already has from that spread; the spread/override is dead code (`cloned` alone already satisfies `MessageTemplateSnapshot`).
  >
  > - 🟡 MINOR — `optionalColorSchema` (`document.ts:11-16`) has no `.catch()`, unlike every sibling color schema (`colorSchema` etc.). An out-of-pattern `textColor` on a button block would throw inside `emailBuilderDocumentSchema.parse()` and fail the *entire* document with the generic "Builder JSON is invalid." (`actions.ts:409-414`) instead of falling back the way `backgroundColor`/`divider.color`/etc. do. Currently unreachable from the shipped UI (button `textColor` isn't editable and is hardcoded to `#ffffff` in `newBlock`, `email-template-builder.tsx:34`), but it's a latent inconsistency in the catch-and-fallback pattern the rest of the schema relies on.
  >
  > - 🟢 LOW — `mjml2html` is called via a double `as unknown as (input: string, options?: MJMLParsingOptions) => Promise<MJMLParseResults>` cast (`lib/email-builder/render.ts:87`). `@types/mjml-core` already declares `mjml2html` as `(input: string | MJMLJsonObject, options?: MJMLParsingOptions) => Promise<MJMLParseResults>`, so this cast may be unnecessary type-erasure — worth a quick check on whether it's needed, since routing through `unknown` would hide a real signature drift on a future `mjml`/`@types/mjml` bump.
  >
  > - 🟢 LOW — Possible snapshot race: `snapshotMessageTemplate`'s upsert on `(templateId, version)` (`actions.ts:240-269`) uses `update: {}`, a silent no-op on conflict. Two concurrent saves on the same template both read the same `template.version`, so the second writer's snapshot silently no-ops while its content update to `messageTemplate` still lands — an intermediate version's content is never recorded anywhere. Standard last-write-wins risk for an admin tool, not unique to this feature.
  >
  > **Sidebar (not tagged for audit):** `parseTokenJson` (`modules/communications/actions.ts:132-149`) and `parsePreviewTokens` (`modules/communications/page.tsx:118-132`) are near-duplicate JSON-token-parsing helpers — same shape and validation, only the entry-filter predicate differs slightly. Pre-existing rather than part of this slice's diff, but a clean "never duplicate logic" consolidation target (e.g. one shared helper in `lib/email`).
  >
  > **Status: `READY-FOR-REVIEW`**
  >
  > **🔧 PATCHER · Claude [06-14-26]:** No reviewer in this rotation → straight to PATCHER. @user adjudicated the two 🟠 scope-intent findings directly: **system (keyed) email templates should stay editable, and every email should be editable in ONE place.** That decision reframes the fix as a *consolidation*, not a lockdown. Committed alongside the prior 06-14 batch (per @user). `npx tsc --noEmit`, `eslint`, and `npm run build` all green.
  > - **🟠 #1 (missing system-template `key` guard) — resolved by decision, not a guard.** Per @user, no key-block was added; the visual builder remains the editor for all 13 system-keyed EMAIL templates (the 5 non-booking ones had no other edit surface). The real risk it pointed at — a *second, conflicting* editor — is removed by the consolidation below.
  > - **🟠 #2 (silently-dead `htmlBody`) — resolved by removing the competing editor.** The standalone "Booking email settings" panel (`updateBookingTemplateSettingsAction` + its raw `htmlBody` textarea) was the surface that silently went inert once `builderJson` was set. That panel/action/schema and the now-orphaned `modules/communications/booking-templates.ts` helper are **deleted**; the visual builder is now the single editor for all EMAIL templates, and version history + restore moved into the builder panel (`page.tsx`). `htmlBody` is now only ever the builder-rendered cache (`updateMessageTemplateBuilderAction`), so there is no second "save" that does nothing.
  > - **🟡 #3 (duplicated `hasBuilderJson`) — deduped.** One canonical `hasBuilderJson` is now exported from `lib/email/render.ts` and used by both `renderEmailTemplate` (replacing the inline check) and `modules/communications/page.tsx` (local copy removed).
  > - **🟡 #4 (restore: no guard + no token re-validation) — token re-validation added.** Per the keep-editable decision, no key-block on restore; but `restoreMessageTemplateVersionAction` (`actions.ts`) now re-runs `assertTemplateTokens` against the restored version's own token metadata (builder tokens via `extractEmailBuilderTokens`/`normalizeEmailBuilderDocument` when `builderJson` is present, else `htmlBody`) before the snapshot took live — an older-allowlist version can't bypass validation.
  > - **🟡 #5 (clone dead spread) — removed.** `snapshotMessageTemplate({ ...cloned, builderRenderer: cloned.builderRenderer }, ...)` → `snapshotMessageTemplate(cloned, ...)`; `cloned` already satisfies `MessageTemplateSnapshot`.
  > - **🟡 #6 (`optionalColorSchema` missing `.catch()`) — added** `.catch("#ffffff")` (`lib/email-builder/document.ts`), matching the catch-and-fallback pattern of every sibling color schema; an out-of-pattern `textColor` no longer fails the whole document parse.
  > - **🟢 #7 (`mjml2html` `as unknown as` cast)** and **🟢 #8 (snapshot last-write-wins race)** left as-is — forward items. The cast is left untouched deliberately (changing it risks a real type break on a future `mjml` bump; needs a deliberate types check, out of scope for this pass); the snapshot race is standard admin-tool last-write-wins.
  > - **Sidebar (`parseTokenJson`/`parsePreviewTokens` near-dup)** left as-is — pre-existing, spans a server action vs. a client page helper; consolidation is its own cleanup, not part of this slice.
  >
  > **Status: `READY-FOR-CONFIRM`**
  >
  > **✅ VALIDATOR · Claude [06-14-26]:** All 8 findings re-verified in code (not on report); full-repo `tsc --noEmit` exits 0, so the file deletions left no dangling references. Each finding, by reference:
  > - 🟠 #1 (system-template `key` guard) — **resolved by @user decision, verified.** `updateMessageTemplateBuilderAction` (`modules/communications/actions.ts:309-385`) loads `findFirst({id,siteId,channel:EMAIL})` with no `key` check (`:313-318`), intentionally keeping all 13 system EMAIL templates editable in one place. Safety nets present: required-token enforcement via `assertTemplateTokens` (`:365`), an at-least-one-renderable-block guard (`:354-356`), and a pre-save version snapshot (`:367`).
  > - 🟠 #2 (silently-dead `htmlBody`) — **VERIFIED.** The competing editor is gone: `modules/communications/booking-templates.ts` is deleted and `updateBookingTemplateSettingsAction`/`bookingTemplateKeySet` are grep-clean across the whole repo (only this ledger still names them). The visual builder is the single editor; `updateMessageTemplateBuilderAction` writes `htmlBody: renderedBuilderHtml` (`actions.ts:375`), so `htmlBody` is only ever the builder-rendered cache — no second "save" that no-ops.
  > - 🟡 #3 (duplicated `hasBuilderJson`) — **VERIFIED.** One exported predicate in `lib/email/render.ts:30-32`, consumed by `renderEmailTemplate` (`render.ts:74`) and imported into `modules/communications/page.tsx:12` (used `:299-300`); the local copy is removed.
  > - 🟡 #4 (restore re-validation) — **VERIFIED.** `restoreMessageTemplateVersionAction` recomputes used tokens from the version itself (builder tokens via `extractEmailBuilderTokens(normalizeEmailBuilderDocument(version.builderJson))` when `hasBuilderJson`, else `htmlBody` tokens) and runs `assertTemplateTokens` against `templateAllowedTokens(version)` BEFORE the snapshot + go-live update (`actions.ts:490-511`).
  > - 🟡 #5 (clone dead spread) — **VERIFIED.** `actions.ts:444` is now `snapshotMessageTemplate(cloned, "Initial cloned version")`; the `{ ...cloned, builderRenderer }` re-spread is gone.
  > - 🟡 #6 (`optionalColorSchema` `.catch()`) — **VERIFIED.** `lib/email-builder/document.ts:11-17` now ends `.catch("#ffffff")`, matching `colorSchema` (`:6-10`) and `fontFamilySchema` (`:18-27`); an out-of-pattern `textColor` falls back instead of failing the whole document parse.
  > - 🟢 #7 (`mjml2html` `as unknown as` cast, `lib/email-builder/render.ts:87`) and 🟢 #8 (snapshot last-write-wins, `actions.ts:236` `update:{}`) — confirmed **deliberately left** as forward items, matching the PATCHER note.
  > - 🟢 fwd/residual: with all 13 system EMAIL templates now editable through one builder and no read-only guard, a malformed builder save on a transactional template (e.g. `order.receipt.customer`, `billing.document.customer`) could degrade a live email. Mitigated by token validation + non-empty-block guard + version history/restore, but a future "you are editing a system template" confirm-step or preview-before-publish would harden the consolidation @user chose.
  >
  > **Status: `CONFIRMED`**
