...
| 1b | Embed/API — public scheduling API (E2) | 🛠 RESOLVED | PATCHER resolved the E2 audit findings: public availability now exposes only available/bookable slots with no native messages, unavailable slots, availability flags, or reason strings; expected native booking validation/conflict failures are mapped to public `EmbedRequestError` statuses while unexpected exceptions get a generic 500 body; and honeypot JSON is parsed/discarded before the `booking_submission` limiter. Awaiting validator confirmation. | 06-15-26 |
