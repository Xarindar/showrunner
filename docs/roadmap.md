...
| 1b | Embed/API — public scheduling API (E2) | 🔵 READY-FOR-AUDIT | `services`/`availability`/`bookings` now live under the E1 gateway (`scheduling:read/write`), with key-derived site tenancy, CORS, public rate limits, service + staff/resource metadata, slot diagnostics, and booking/request creation through the native availability + Serializable booking engine (no fork). Verification: `prisma validate`, `prisma generate`, `tsc --noEmit --pretty false`, touched-file ESLint, and runtime smoke for OPTIONS/services/availability/invalid booking/full booking create. | 06-14-26 |
