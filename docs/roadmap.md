...
| 2 | Scheduling — booking reminders (scheduled sweep) | 🔍 AUDITED | strong idempotent design: `BookingReminder` unique-per-booking claim w/ stale-CLAIMED recovery + `queueEmail` idempotency key → no double-send even across a crash; site-aware, owner-configurable (enabled + lead), reuses email outbox (no fork). worker-3's `EmailOutboxStatus.CLAIMED` flag was a misread (it's `BookingReminderStatus.CLAIMED`, valid, build green). 🟡 FAILED reminders never retried (transient queue failure permanently drops a reminder) → READY-FOR-PATCH; 🟢 100/sweep cap, cron is deploy-config. Committed `7329f4e` | 06-13-26 |
