...
| 2 | Scheduling — calendar adapters (ICS feed + add-to-calendar) | ✅ CONFIRMED | `icsCalendarAdapter` (native stays default); HMAC token-protected read-only `/api/calendar/feed.ics` (site + per-staff) and `/api/calendar/booking.ics`; tokens scoped + timing-safe, routes fail closed on bad token/missing entity/scheduling-disabled, booking route derives siteId from the record (no cross-site), `icsText` escapes `\`/newline/`;`/`,` (injection-safe), secret fail-closed in prod, GET-only/bounded. 🟢 fwd: no weak-secret strength guard; line-fold by char not octet; bearer-token feeds over a single global secret → add a per-site salt for independent revocation. chunk-2 Google Calendar free/busy 🔍 AUDITED → READY-FOR-PATCH (`2bb7b5b`): admin-gated OAuth + `expectedSiteId`, AES-256-GCM tokens w/ auto-refresh, tenancy-scoped freeBusy, busy windows exclude slots; 🟡 fail-closed on freebusy hard-error blocks ALL of an owner's slots (treat error as "couldn't check" → degrade to native, not "busy all day"). Cal.com/Outlook pending. Committed `0bd49d2` | 06-13-26 |
