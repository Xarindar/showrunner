...
| 3/5 | Commerce / Gallery — fulfillment exports + print/photo lab handoff | 🔵 READY-FOR-AUDIT | `8f7ec27` (worker-4). Added order-level fulfillment export + print/photo lab handoff metadata; `/admin/modules/products/fulfillment-export` CSV for paid physical order items; derived lab/gallery/media metadata from existing `Product.attributes` keys and resolves referenced gallery/media labels where present; audited admin actions mark paid physical orders exported and record print-lab handoff for paid/fulfilled physical orders; Products admin exposes fulfillment CSV, selected-order export batch, and print-lab handoff controls. Checkout totals/payment settlement untouched; fulfillment status transition remains through the dedicated fulfillment action from `baf2296`. Validation reported: prisma generate/validate, focused eslint, lint, tsc, build, diff checks, `git show --check`. Not yet audited | 06-13-26 |
...
| 2 | Scheduling — waitlist + request-only approval | 🔵 READY-FOR-AUDIT | `3c1ec2b` (worker-2). Added `Service.requestOnly`, `Service.waitlistEnabled`, `BookingWaitlistStatus`, and `BookingWaitlistEntry`; public booking keeps the existing availability + Serializable booking adapter path, creates `PENDING` requests for request-only services, offers waitlist join when no slots exist, and stores service/date/staff preference/contact/intake/policy data. Appointments admin waitlist desk can promote/decline; promotion calls `nativeSchedulingAdapter.createBooking` with `CONFIRMED` so conflict/resource/staff checks are reused. Waitlist access uses `getAccessibleBookingWaitlistWhere`; request approval still uses existing `updateBookingStatusAction`; lifecycle events added for request approved + waitlist joined/promoted/declined. Validation reported: prisma generate/validate, tsc, focused eslint, lint, build, diff checks, `git show --check`. Not yet audited | 06-13-26 |
...
| 7 | Forms — file-upload fields | 🔵 READY-FOR-AUDIT | `fc0e5f5` (worker-1). Added `FormFieldType.FILE`, owner MIME allowlist/max-size config via existing `FormField.validationRules`, public multipart file input support, client-side UX validation, and authoritative server-side honeypot/rate-limit/visibility/required/MIME/size validation before storage. Reuses existing `MediaAsset`/R2 primitives through an opt-in private-file upload mode; submission JSON stores bounded file metadata only. Added gated admin download route requiring `forms:manage`, submission data scope, and asset-reference verification before serving private media through `mediaDeliveryResponse`; admin submission detail shows file download links and metadata. Validation reported: prisma validate/generate, tsc, focused eslint, lint, build, diff checks, `git show --check`. Not yet audited | 06-13-26 |
...
| 10 | Communications — delivery-status hardening | 🔵 READY-FOR-AUDIT | `64a8d1c` (worker-3). Provider webhook normalizes common provider payload keys/event aliases, maps delivered/delayed/bounced/complained/unsubscribed into `EmailOutbox` status/failure visibility and suppression/subscriber updates, and Communications outbox admin shows latest provider event + last error with manual resend for non-active rows. Reuses existing provider-events endpoint, EmailOutbox queue path, suppression checks, and admin surfaces; no parallel message log or send engine. Validation reported: focused eslint on 4 files, `git show --check`, repo lint, tsc, build. Not yet audited | 06-13-26 |
