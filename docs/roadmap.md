...
| 3/5 | Commerce / Gallery — fulfillment exports + print/photo lab handoff | 🛠 RESOLVED | `8f7ec27` (worker-4) + patch `a5ffc52` (claude-patcher). Order-level fulfillment export + print/photo lab handoff metadata; `/admin/modules/products/fulfillment-export` CSV for paid physical order items; audited mark-exported + print-lab-handoff actions. 🛠 PATCHED → READY-FOR-CONFIRM (3×🟢): re-export-everything addressed via opt-in `?unexportedOnly=1` delta export + "Export new orders only" button (default unchanged, filter audit-logged); download-doesn't-flag and site-level order actions confirmed intentional/not-a-regression (confirm-with-note). CSV formula-injection-safe, tenancy-scoped, admin-gated, audit-logged. Awaiting validator | 06-13-26 |
...
| 2 | Scheduling — waitlist + request-only approval | 🛠 RESOLVED | `3c1ec2b` (worker-2) + patch `6872aef` (claude-patcher). Added `Service.requestOnly`, `Service.waitlistEnabled`, `BookingWaitlistStatus`, and `BookingWaitlistEntry`; public booking keeps the existing availability + Serializable booking adapter path, creates `PENDING` requests for request-only services, offers waitlist join when no slots exist. Appointments admin waitlist desk can promote/decline; promotion reuses `nativeSchedulingAdapter.createBooking`. 🛠 PATCHED → READY-FOR-CONFIRM: 🟠 HIGH closed — honeypot (`companyWebsite`) + per-IP `publicRateLimitMessage` on BOTH `joinPublicWaitlistAction` (`waitlist_join`) and `createPublicBookingAction` (`booking_submission`, the sidebar gap); 🟡 closed — shared `upsertPublicClient` (create-or-fill-blank, never overwrites stored name/phone) reused by native booking + waitlist; 🟡 closed — promotion now atomically claims `WAITING→PROMOTED` via conditional `updateMany` before create, releases on failure. 🟢 waitlist-scope-on-preference left as a confirm-with-note (data-scope intent). Awaiting validator | 06-13-26 |
...
| 7 | Forms — file-upload fields | 🛠 RESOLVED | `fc0e5f5` (worker-1) + patch `18cc1e4` (claude-patcher). Added `FormFieldType.FILE`, owner MIME allowlist/max-size config, public multipart upload, authoritative server-side honeypot/rate-limit/visibility/required/MIME/size validation before storage, gated record-scoped/asset-verified admin download route. 🛠 PATCHED → READY-FOR-CONFIRM: 🟡 orphaned-uploads closed — submission now tracks uploaded asset ids + `submissionPersisted` flag and deletes orphans on any pre-persist failure (new R2 `delete` impl + reusable `deleteMediaAsset` helper, storage object + row, siteId-scoped, never-throws); 🟢 aggregate cap added (25MB total across FILE fields). Magic-byte MIME re-check, SVG excluded, private storage. Awaiting validator | 06-13-26 |
...
| 10 | Communications — delivery-status hardening | 🛠 RESOLVED | `64a8d1c` (worker-3) + patch `8a1bd67` (claude-patcher). Provider webhook normalizes payload keys/event aliases, maps delivered/delayed/bounced/complained/unsubscribed into `EmailOutbox` status/suppression, admin shows latest event + manual resend. 🛠 PATCHED → READY-FOR-CONFIRM (3×🟢): out-of-order DELIVERED-after-bounce fixed — a late DELIVERED now records `deliveredAt` only and preserves the terminal FAILED status + lastError when `bouncedAt`/`complainedAt` is set; orphan-event siteId fallback and resendable-SENT confirmed intentional (confirm-with-note). Bounce→FAILED safe (worker never re-selects FAILED); manual resend re-checks suppression. Awaiting validator | 06-13-26 |
