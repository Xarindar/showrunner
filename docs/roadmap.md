...
| 1b | Embed/API — site keys + public API gateway (E1) | 🔧 READY-FOR-CONFIRM | PATCHER fixed the origin boundary: publishable keys now reject originless requests unless `allowServerToServer` is explicitly enabled, present-but-invalid/opaque/`null` Origins fail closed, invalid/missing key attempts hit cheap format validation + a global/IP limiter before key lookup, revocation requires typed server-side confirmation, and rejected origins no longer get ACAO reflected. Migration `20260614153000_embed_api_key_server_to_server_gate` applied to local Postgres; `/api/public/v1/ping` OPTIONS/GET matrix passed for allowed, disallowed, missing, malformed/null, invalid, revoked, and explicit s2s cases | 06-14-26 |
