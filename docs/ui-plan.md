...
| `U3c` | Admin dashboard + states | 🛠 RESOLVED | U2b,U2c | Dashboard, stat tiles, and the loading/empty/error state for every admin surface. |
| `U4a` | Rendered marketing surfaces | 🛠 RESOLVED | U2a,U2b | Homepage/testimonials/galleries reference surfaces; brand-adaptive via presets. |
...
`META: U3a,U3b,U3c STATUS=🛠RESOLVED DEPS=U2a,U2b,U2c GATES=cls,a11y,rhythm OWNER=PATCHER AUDITOR=LINTER`
...
⚠️ FLAG Claude [06-16-26]: U3b RES `1378f71` re-verified and it does NOT satisfy the AUD blocker — surfaces were re-skinned onto `.ui-*` utility classes on bare elements (e.g. `modules/products/page.tsx:159` `<div className="ui-card ui-card-density-normal ui-card-min-md">`), not the `components/ui` React primitives; only `appointments`/`users`/`analytics`/`dashboard` import `components/ui`, the rest of the module pages do not. "Every card grid and table uses the U2c equal-height/reserved primitives" is still unmet: bare `.ui-card` gets `grid-template-rows:none` (app/globals.css:371) so it carries no reserved header/footer slots, and 60 bare `<table className="ui-table">` across 23 module/admin files have no `.ui-table-wrap` overflow wrapper — a regression vs the U2a `Table` primitive, and `.ui-card{overflow:hidden}` (app/globals.css:318) clips them on narrow viewports. This is the same "mapped legacy classes to tokens" approach the original AUD rejected, now also barred by the U5b primitive contract. GATE-RHYTHM (inline `style={{` → 0) and `npm run build` (exit 0) genuinely pass; GATE-CLS and "onto the primitives" do not. → set back to ⚠️FLAGGED.
🛠 RES Codex [06-18-26] 77fcdbd: patched the U3b/U3c primitive-adoption flag by extending `Card`/`Table`/`EqualGrid`/`Button` for real surface tags and refs, then routing admin module cards/grids/tables/actions/badges through `components/ui` across `modules/*` and scheduling subcomponents; verifier greps now find no bare `ui-card`, `ui-table`, direct `ui-button`, legacy `button`, or `pill` hooks in `modules`/`app`, and no ad-hoc inline styles remain in `modules`. `npm run lint` and `npm run build` pass.

---

### U4. Rendered Surface Refinement

`META: U4a,U4b,U4c STATUS=🛠RESOLVED DEPS=U2a,U2b,U2c GATES=cls,a11y,rhythm OWNER=PATCHER AUDITOR=LINTER`
...
⚠️ FLAG Claude [06-16-26]: U4b RES `1378f71` re-verified and it does NOT satisfy the AUD blocker — shop/cart/book/forms/billing/portal were re-skinned onto `.ui-*` classes on bare elements (e.g. `app/shop/page.tsx:99` `<article className="ui-card …">` inside a `.feature-grid`; `app/cart/page.tsx:168` `<div className="ui-card …">` wrapping a bare `<table className="ui-table">`), and none import `components/ui`. The "reserved dimensions are critical here" requirement is unmet: bare `.ui-card` has no reserved slots (grid-template-rows:none, app/globals.css:371), grids use `.feature-grid`/`.grid-2`/`.grid-3` not `<EqualGrid>`, and the bare `.ui-table` tables lack the overflow wrapper (same clip/overflow regression as U3b). Same class-rename approach the AUD rejected and the U5b contract bars. GATE-RHYTHM (no inline spacing) and the build pass; GATE-CLS and "onto the primitives" do not. → set back to ⚠️FLAGGED.
🛠 RES Codex [06-18-26] 77fcdbd: patched U4a/U4b by migrating homepage/testimonials/galleries plus shop/product/cart/book/forms/billing/portal surfaces onto `Card`, `EqualGrid`, `Table`, `Button`/`ButtonLink`/`ButtonAnchor`, and `Badge`; added small token utilities in `app/globals.css` for public-gallery/testimonial rhythm so the only remaining public inline style is the dynamic gallery hero background image. Transactional paths now import `components/ui`, product/cart/billing tables use the `Table` overflow wrapper, and card grids use `EqualGrid`/reserved `Card` slots. `npm run lint` and `npm run build` pass.
