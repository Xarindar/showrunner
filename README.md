...
The payments platform is generalized behind `lib/payments/` so gateways can connect per site. The primary onboarding path is **one-click OAuth** through the standalone AdmitOne Connect broker: the browser receives only an opaque single-use code, and Showrunner redeems it over a signed back channel (`lib/payments/connect/`, routes under `/api/payments/connect/{provider}/{start,callback}`). Tokens are stored encrypted via `PAYMENT_CREDENTIAL_ENCRYPTION_KEY` and used for direct charges on the owner's own account — the broker is never in the payment path. Configure it with `ADMITONE_CONNECT_BASE_URL`, `ADMITONE_CONNECT_CLIENT_ID`, and `ADMITONE_CONNECT_SHARED_SECRET`; when unset, onboarding falls back to the paste-your-own-keys wizard (PayPal is always paste-based by design). Stripe's checkout webhook is created automatically and upstream authorization is revoked before local disconnect. Square requires an explicit site-bound location choice; its webhook signature key is pasted once under Payments → Square → Manage. Run `npm run payments:refresh-square` daily: OAuth credentials refresh at least every seven days and the worker fails when a connection is stale for eight days. See `docs/PAYMENTS_SETUP.md`.
