...
The payments platform is generalized behind `lib/payments/` so gateways can connect per site. The primary onboarding path is **one-click OAuth** through the standalone AdmitOne Connect broker: the owner clicks Connect, signs in on Stripe/Square's hosted page, and the broker hands this deployment its own merchant tokens (`lib/payments/connect/`, routes under `/api/payments/connect/{provider}/{start,callback}`). Tokens are stored encrypted via `PAYMENT_CREDENTIAL_ENCRYPTION_KEY` and used for direct charges on the owner's own account — the broker is never in the payment path. Configure it with `ADMITONE_CONNECT_BASE_URL`, `ADMITONE_CONNECT_CLIENT_ID`, and `ADMITONE_CONNECT_SHARED_SECRET`; when unset, onboarding falls back to the paste-your-own-keys wizard (PayPal is always paste-based by design). Stripe's checkout webhook is created automatically during connect; Square's webhook signature key is pasted once under Payments → Square → Manage. Square OAuth tokens expire (~30 days) — schedule `npm run payments:refresh-square` daily to refresh them through the broker ahead of expiry. Stripe adds owner-controlled payment-method toggles for card-backed Apple Pay and Google Pay, Cash App Pay, Klarna, and Affirm. See `docs/PAYMENTS_SETUP.md`. Apple Pay domain registration uses the deployment's public URL; each client's custom domain must be registered for reliable Apple Pay on owner domains.
