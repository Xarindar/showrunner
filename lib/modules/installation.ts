...
export async function ensureModuleInstallations(seedEnabled: ModuleId[]): Promise<void> {
  ...
  await prisma.$transaction(
    moduleRegistry.map((module) =>
      prisma.moduleInstallation.upsert({
        ...
        create: { moduleId: module.id, installed: true, enabled: seed.has(module.id) }
      })
    )
  );
}
...
export async function setModuleEnablement(enabledIds: ModuleId[]): Promise<void> {
  ...
  await prisma.$transaction(
    moduleRegistry.map((module) =>
      prisma.moduleInstallation.upsert({
        ...
        create: { moduleId: module.id, installed: true, enabled: enabled.has(module.id) }
      })
    )
  );
}
...
export async function resolveEnabledModuleIds(legacyEnabled: ModuleId[]): Promise<ModuleId[]> {
  let rows: { moduleId: string }[];

  try {
    rows = await prisma.moduleInstallation.findMany({ where: { enabled: true }, select: { moduleId: true } });
  } catch {
  ...
  if (rows.length === 0) {
    ...
    await ensureModuleInstallations(legacyEnabled).catch(() => {});
    return normalizeModules(legacyEnabled);
  }
